...
def _run_foreground(
    ...
    for retry_count in range(max_retries + 1):
        try:
            ...
            from tools.terminal_execution_scope import terminal_process_was_started
            if terminal_process_was_started():
                return _error_json("The admitted command lost its outcome; inspect existing effects before retrying.", status="uncertain")
...
def terminal_tool(
    ...
    try:
        ...
        from tools.terminal_execution_scope import terminal_execution_scope
        with terminal_execution_scope(command, env_type, background=background):
