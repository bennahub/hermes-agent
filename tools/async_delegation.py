    ...
    with _DB_LOCK, _transaction() as conn:
        ...
        for delegation_id, payload, completed_at, dispatched_at in rows:
            ...
            if isinstance(evt, dict):
                # Older profile pollers could inject successfully but ACK the
                # launch ledger. Reconcile only an exact verified Owner final;
                # hidden-only/interrupted work still gets native recovery.
                from agent.autonomy.owner_continuity import completed_child_delivery
                if completed_child_delivery(delegation_id, evt.get("parent_session_id")):
                    conn.execute("""UPDATE async_delegations SET delivery_state='delivered',
                                  delivered_at=?, updated_at=?, delivery_claim=NULL, delivery_claimed_at=NULL
                                  WHERE delegation_id=? AND delivery_state='pending'""", (now, now, delegation_id))
                    continue
                evt["restored"] = True
