...
class ProcessSession:
    ...
    owner_continuity_work_id: str = ""
    owner_continuity_home: str = ""
    execution_scope_locator: Optional[dict] = None
    ...
    def mark_exited(self, exit_code, reason: str = "exited", source: str = "") -> None:
        ...
        self.exited = True
        from tools.terminal_execution_scope import close_process_execution_scope
        close_process_execution_scope(self)
...
_CHECKPOINT_FIELDS = (
    ...
    *(f"watcher_{k}" for k in _WATCHER_ROUTE_KEYS), "watcher_interval",
    ...
    "owner_continuity_work_id", "owner_continuity_home", "execution_scope_locator")
...
class ProcessRegistry:
    ...
    def _new_session(command, task_id, owner_task_id, session_key, cwd, **extra) -> ProcessSession:
        from tools.terminal_execution_scope import native_process_fields
        scoped = native_process_fields()
        return ProcessSession(
            id=scoped.pop("id", f"proc_{uuid.uuid4().hex[:12]}"), **scoped, command=command, task_id=task_id,
    ...
    def _track_started(self, session: ProcessSession, reader_target, reader_name: str, extra_args=()) -> None:
        ...
        from tools.terminal_execution_scope import native_process_started
        native_process_started(session)
    ...
    def _move_to_finished(self, session: ProcessSession):
        ...
        if was_running and session.owner_continuity_work_id:
            try:
                from agent.autonomy.owner_continuity import signal_process_completion
                signal_process_completion(session)
            except Exception:
                logger.exception("Owner task process completion could not be recorded; its deadline remains durable")
        self._write_checkpoint()
        # Structured native continuations receive their durable completion through
        # owner_continuity.signal_process_completion. Do not enqueue the legacy
        # callback as a second parent turn. Fields are immutable routing ownership
        # stamped at bind time; unowned legacy sessions keep this path.
        if was_running and session.notify_on_complete and not (
            session.owner_continuity_work_id and session.owner_continuity_home
        ):
            notification = {
                "type": "completion",
                "session_id": session.id,
