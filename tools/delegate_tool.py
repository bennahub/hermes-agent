...
def _build_child_agent(
    ...
    # Derive while the parent dispatch is still admitted, before backgrounding.
    from agent.execution_scope import capture_binding, derive_child

    parent_scope = capture_binding()
    if parent_scope is not None:
        source_key = "delegate:" + subagent_id
        child._pending_inherited_execution_scope = derive_child(parent_scope, source_key, goal)
        child._pending_inherited_execution_scope_source_key = source_key
        child._pending_inherited_execution_scope_goal = goal
