...
from tools.mcp_tool_errors import (
    _is_auth_error, _is_session_expired_error, mcp_connection_tool_error)

logger = logging.getLogger("tools.mcp_tool")
_MISSING = object()

# Last-resort wording for an auth failure the Connection taxonomy could not describe (no descriptor
# could be built at all). Still no operator instructions: `hermes mcp login <server>` and "delete the
# tokens file under ~/.hermes/mcp-tokens/" are things the MODEL cannot do, and the Owner receives
# them second-hand as the agent telling them to open a terminal. Connections owns the recovery.
_NEEDS_REAUTH_MSG = (
    "The '{s}' connection is disconnected and only the Owner can restore it, in Connections. Do NOT "
    "retry this tool and do NOT ask the user to run terminal commands — continue with what you can "
    "do without it, or stop and say the work is blocked on that connection.")
...
def _handle_auth_error_and_retry(server_name: str, exc: BaseException, retry_call, op_description: str):
    ...
    failure records a canonical Connection fault and returns the structural ``connection`` payload,
    so the model stops refreshing AND the Connections row stops calling a revoked token configured."""
    ...
    if recovered:
        ...
        if result is not None:
            return result
    # Recovery is exhausted: this is now a Connection only the Owner can restore. Publish it to the
    # canonical health store (local metadata cannot see a revoked-but-present token) and hand the
    # model the structural verdict rather than the provider's words or a shell command.
    payload, _block = mcp_connection_tool_error(server_name, exc)
    if payload is not None:
        _core._bump_server_error(server_name)
        return payload
    return _strike(server_name, _NEEDS_REAUTH_MSG.format(s=server_name),
                   needs_reauth=True, needs_owner=True, retryable=False, server=server_name)
...
def _dispatch(server_name: str, server: Any, op: str, call, tool_timeout: float, recoverers,
    ...
    try:
        ...
        # A 401/403 that the recovery ladder's narrow type-gate did not claim still must not reach
        # the model as the provider's own words. ``_sanitize_error`` only redacts credential-SHAPED
        # substrings, so an auth body full of ordinary prose passed straight through, and the model
        # paraphrased it to the Owner as an answer.
        auth_payload, _auth_block = mcp_connection_tool_error(server_name, exc)
