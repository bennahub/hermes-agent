...
def _reset_server_error(server_name: str) -> None:
    ...
    Also clears any recorded Connection fault for this server. This is the recovery half of the MCP
    Connection contract and it is not optional: without it a reconnected MCP server would stay
    marked broken in Connections forever — the same divergence as a revoked token reading
    ``configured``, with the signs reversed. Short-circuited by a ``stat`` of the health store on the
    common (no faults recorded) path.
    """
    _server_error_counts[server_name] = 0
    _server_breaker_opened_at.pop(server_name, None)
    try:
        from agent.connection_health import _known_empty, current_profile_scope, record_healthy
        # The scope has to be passed explicitly (an MCP grant is per-profile, so ``owning_scope``'s
        # auth.json probe would resolve the wrong key), so take the store's own emptiness
        # short-circuit first: the healthy path then costs one ``stat``, not a path resolution per
        # successful tool call.
        if not _known_empty():
            record_healthy(str(server_name or "").strip(), current_profile_scope())
    except Exception:
        pass
