...
def _roster(root: Path) -> list[tuple[str, Path]]:
    """Live visible profiles, shared by the prompt, DM and autonomy rollout."""
    ...
    def visible(home):
        try:
            import yaml
            data = yaml.safe_load((home / "profile.yaml").read_text()) or {}
            return (data.get("ui_meta", {}).get("hermes-bots", {}).get("hidden") is not True)
        except (OSError, ValueError, AttributeError, yaml.YAMLError):
            return True  # Missing metadata keeps ordinary CLI profiles usable.

    entries = [("default", root)] if root.is_dir() and visible(root) else []
    try:
        for child in sorted((root / "profiles").iterdir()):
            try:
                validate_profile_name(child.name)
            except ValueError:
                continue
            if (child.name != "default" and child.is_dir()
                    and not named_profile_is_deleted(child) and visible(child)):
