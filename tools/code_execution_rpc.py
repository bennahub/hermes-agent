...
def _default_dispatch(task_id):
    ...
    # RPC workers may outlive the creating call's thread. Bind its immutable
    # authority now; each request gets its own context copy for concurrent use.
    authority_context = contextvars.copy_context()
    def dispatch(tool_name, tool_args):
        from tools.code_execution_identity import run_in_context
        return run_in_context(authority_context, handle_function_call,
...
def _handle_rpc_request(request: dict, *, allowed_tools: frozenset, tool_call_counter: list,
    ...
    try:
        from tools.code_execution_identity import request_identity
        # Legacy native callers have no seq; their host counter identifies one
        # dispatch. Generated transports include seq, stable across reconnect.
        sequence = request.get("seq", tool_call_counter[0])
        with thread_scoped_silence(), request_identity(sequence):
