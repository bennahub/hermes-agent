...
def _unattended_deny(command: str, ctx: _Unattended) -> dict | None:
    """Deny-mode handling for unattended contexts; scoped recoverable findings use the guardian."""
    if ctx.mode() != "deny":
        return None
    ...
    def scoped_guardian(description: str, pattern_key: str, pattern_keys: list[str]) -> dict | None:
        try:
            from agent.execution_scope_observations import approval_context as scope_context
            if not scope_context():
                return None
        except Exception:
            return None
        result, _smart_denied = _smart_gate(
            _COMMAND_GATE, command=command, description=description,
            pattern_key=pattern_key, pattern_keys=pattern_keys,
            session_key=get_current_session_key(), human_present=False,
        )
        # ESCALATE has no unattended approval surface and therefore fails closed.
        return result if result is not None else block(description)

    is_dangerous, pattern_key, pattern_description = detect_dangerous_command(command)
    warnings: list[tuple[str, str]] = []
    if is_dangerous:
        warnings.append((pattern_key, f"Command flagged as dangerous ({pattern_description})"))
    ...
    try:
        ...
        if _tirith_fail_open():
            ...
            # Missing Tirith remains fail-closed even when a recoverable pattern exists.
    ...
    if tirith and tirith.get("action") in ("block", "warn"):
        description = _format_tirith_description(tirith)
        findings = tirith.get("findings") or []
        rule_id = findings[0].get("rule_id", "unknown") if findings else "unknown"
        warnings.append((f"tirith:{rule_id}", description))

    if not warnings:
        return None
    description = "; ".join(item[1] for item in warnings)
    keys = [item[0] for item in warnings]
    result = scoped_guardian(description, keys[0], keys)
    if result is not None:
        if ctx.name == "single_query" and is_dangerous:
            result.setdefault("pattern_key", pattern_key)
            result.setdefault("description", pattern_description)
        return result
    result = block(description)
    if ctx.name == "single_query" and is_dangerous:
        result.update(pattern_key=pattern_key, description=pattern_description)
    return result
