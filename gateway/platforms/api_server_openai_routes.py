...
class OpenAICompatRoutesMixin:
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if not messages or messages[-1].get("role") != "user" or not conversation_messages:
            return _invalid_request("The current final message must be an original user instruction")
        ...
        run_kwargs = dict(
            execution_source=original_request(user_message, source="api_current_input"),
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        if not input_messages or input_messages[-1]["role"] != "user":
            return _error_response("The current final input must be an original user instruction", 400)
        if not _content_has_visible_payload(user_message):
