...
async def _handle_runs(self, request: "web.Request", *, _api_server) -> "web.Response":
    ...
    if isinstance(raw_input, str):
        ...
        if not isinstance(raw_input, list) or not raw_input or not isinstance(raw_input[-1], dict) or raw_input[-1].get("role", "user") != "user":
            return _json_error(_openai_error, "The current final input must be an original user instruction", status=400)
    ...
    launch = _RunLaunch(
        ...
        execution_source=original_request(user_message, source="api_run", source_id=run_id) if room_dispatch is None else None)
...
def _run_agent_sync(self, run: _RunLaunch, agent, approval_notify, *, _api_server):
    ...
    with self._profile_scope(run.request_profile):
        try:
            ...
            stage_original_request(agent, run.execution_source)
...
async def _handle_steer_run(self, request: "web.Request", *, _api_server) -> "web.Response":
    ...
    try:
        ...
        accepted = admit_owner_amendment(agent, steer_text, agent.steer)
    except Exception as exc:
