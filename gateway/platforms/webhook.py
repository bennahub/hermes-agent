...
class WebhookAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._route_inference_cache = {}
        self._route_inference_cache_ttl_seconds = 300.0
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        for name, route in self._routes.items():
            ...
            if bool(provider) != bool(model):
                raise ValueError(f"[webhook] Route {name!r} inference pin requires both provider and model")
            _, error = await self._resolve_route_inference_pin(name, route)
            if error:
                logger.warning("[webhook] Route %s inference prewarm unavailable; retry on POST: %s", name, error)
    ...
    def _reload_dynamic_routes(self) -> None:
        ...
        try:
            ...
            for name, (signature, _, _) in list(self._route_inference_cache.items()):
                if name not in self._routes or self._route_cache_signature(self._routes[name]) != signature:
                    self._route_inference_cache.pop(name, None)
    ...
    async def _handle_webhook(self, request: "web.Request") -> "web.Response":
        ...
        resolved_inference, error = await self._resolve_route_inference_pin(route_name, route_config)
        if error:
            logger.error("[webhook] Route %s inference pin unavailable: %s", route_name, error)
            return _json_error("Route inference configuration unavailable", 503)
        ...
        return self._dispatch_agent_run(request, route_config, route_name, profile, payload, prompt, event_type,
                                        delivery_id, now, resolved_inference=resolved_inference)

    def _dispatch_agent_run(self, request, route_config: dict, route_name: str, profile, payload: Any, prompt: str,
                            event_type: str, delivery_id: str, now: float, *, resolved_inference=None) -> "web.Response":
        ...
        # A webhook is an independent one-shot workload. Copy the pre-resolved
        # route pin into the delivery-unique session, then remove it in
        # on_processing_complete so credentials do not accumulate in memory.
        runner = self.gateway_runner
        if runner is not None and resolved_inference is not None:
            session_key = runner._session_key_for_source(source)
            runner._session_model_overrides[session_key] = dict(resolved_inference)
            ...
            if reasoning_effort is not None:
                ...
                if parsed_reasoning is not None:
                    runner._session_reasoning_overrides[session_key] = parsed_reasoning
        ...
        try:
            ...
            if runner is not None:
                try:
                    ...
                    logger.debug(
                        "[webhook] Failed to clear inference override for %s",
    ...
    @staticmethod
    def _profile_home_for_route(route_config: dict):
        """Return the credential/config home that owns a route pin."""
        ...
        if isinstance(profile, str) and profile.strip():
            ...
            if not profile_exists(profile):
                raise ValueError(f"route profile {profile!r} does not exist")
            return get_profile_dir(profile)
        return get_process_hermes_home()



    def _route_cache_signature(self, route_config: dict) -> tuple[str, str, str]:
        """Build a reload-safe cache signature.

        A malformed/deleted profile must evict that route's cache entry, not
        abort reconciliation of every dynamic route.
        """
        provider = str(route_config.get("provider") or "").strip()
        model = str(route_config.get("model") or "").strip()
        try:
            home = str(self._profile_home_for_route(route_config))
        except (TypeError, ValueError):
            home = "<invalid-route-profile>"
        return provider, model, home



    async def _resolve_route_inference_pin(
        self, route_name: str, route_config: dict
    ) -> tuple[dict | None, str | None]:
        """Resolve a route pin once, returning (override, safe_error)."""
        if route_config.get("deliver_only"):
            return None, None
        ...
        if not provider or not model:
            return None, "both provider and model are required"

        try:
            profile_home = self._profile_home_for_route(route_config)
        except (TypeError, ValueError) as exc:
            return None, str(exc)
        signature = (provider, model, str(profile_home))
        now = time.monotonic()
        cached = self._route_inference_cache.get(route_name)
        if cached and cached[0] == signature and now < cached[2]:
            return dict(cached[1]), None

        from hermes_cli.model_switch import switch_model

        def _resolve_in_profile():
            # Install/reset the target profile scope in the same worker thread
            # that resolves provider config, OAuth and secrets.
            from gateway.run import _profile_runtime_scope
        ...
        resolved = await asyncio.to_thread(_resolve_in_profile)
        if not resolved.success:
            return None, resolved.error_message or "model resolution failed"
        override = {
            "model": resolved.new_model,
            "provider": resolved.target_provider,
            "api_key": resolved.api_key,
            "base_url": resolved.base_url,
            "api_mode": resolved.api_mode,
        }
        self._route_inference_cache[route_name] = (
            signature,
            override,
            now + self._route_inference_cache_ttl_seconds,
        )
