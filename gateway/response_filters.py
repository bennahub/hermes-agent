...
# These spellings are unambiguous machine control tokens. Bare ``SILENT`` and
# ``NO REPLY`` remain valid human prose and are suppressed only on explicitly
# autonomous/background lanes.
UNAMBIGUOUS_SILENCE_CONTROLS = frozenset({"[SILENT]", "NO_REPLY"})
...
def is_unambiguous_silence_control(response: Any) -> bool:
    """True only for exact control-token spellings safe to hide globally."""
    if not isinstance(response, str):
        return False
    stripped = response.strip()
    if not stripped or len(stripped) > 64:
        return False
    return _canonical_silence_candidate(stripped) in UNAMBIGUOUS_SILENCE_CONTROLS
