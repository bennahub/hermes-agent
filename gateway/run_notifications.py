...
class GatewayNotificationsMixin:
    ...
    async def _run_process_watcher(self, watcher: dict) -> None:
        ...
        while True:
            ...
            if session.exited:
                # Native ownership is immutable process routing metadata. Its
                # continuation already received the durable completion, so the
                # independent legacy watcher must not create a duplicate turn.
                if agent_notify and getattr(session, "owner_continuity_work_id", "") and getattr(
                    session, "owner_continuity_home", ""
                ):
                    break
