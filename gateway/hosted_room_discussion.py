...
_USER_PAYLOAD_OPTIONAL = frozenset({"reply_to", "attachments"})
_MEMBER_MESSAGE_OPTIONAL = frozenset({"attachments", "reply_to"})
MAX_EVENT_ATTACHMENTS = 8
_ATTACHMENT_FIELDS = frozenset({"artifact_id", "filename", "mime_type", "size_bytes"})
...
class DiscussionRoom:
    ...
    @property
    def active_members(self) -> tuple[DiscussionMember, ...]:
        """Current turn participants; members retains historical identities for replay."""
...
def validate_user_payload(value: Any, *, has_uploads: bool = False) -> dict[str, Any]:
    """Validate and normalize the exact ``message.user`` Discussion payload."""
    payload = _exact_fields(value, label="user payload", required=_USER_PAYLOAD_FIELDS, optional=_USER_PAYLOAD_OPTIONAL)
    reply_to = validate_reply_to(payload.get("reply_to"))
    attachments = validate_attachments(payload.get("attachments"))
    caption = payload["text"]
    caption = "" if caption == "" and (attachments or has_uploads) else _text(
        caption, label="user payload text", max_bytes=MAX_USER_TEXT_BYTES)
    return {
        "text": caption,
...
def _validate_member(raw: Any, index: int, known_profiles: set[str]) -> DiscussionMember:
    ...
    member = _exact_fields(
        ...
        optional=frozenset({"display_name", "target", "active"}))
    ...
    if not isinstance(active, bool):
        raise DiscussionValidationError(f"member {index} active must be a boolean")
    return DiscussionMember(member_id, profile, handle, display_name, target, active)
...
def validate_roster(value: Any, *, local_profiles: Iterable[str]) -> tuple[DiscussionMember, ...]:
    ...
    if len(value) > hosted_rooms.MAX_MEMBERS:
        raise DiscussionValidationError("too many room members")
    presumed_active = sum(1 for entry in value if not isinstance(entry, Mapping) or entry.get("active", True) is not False)
    if not MIN_DISCUSSION_MEMBERS <= presumed_active <= MAX_DISCUSSION_MEMBERS:
        raise DiscussionValidationError(
...
def _unaddressed_member_mentions(
    ...
    for event in messages:
        ...
        for member in resolve_mentions((str(event.payload["text"]),), room.active_members, default_all=False):
            if member.member_id != speaker_id:
                cited_at[member.member_id] = event.seq
    return tuple(
        member for member in room.active_members
...
def _validate_member_message(kind: str, payload: Payload, actor: Payload, room: DiscussionRoom) -> Payload:
    _exact_fields(payload, label="message.member payload", required=_MEMBER_MESSAGE_FIELDS, optional=_MEMBER_MESSAGE_OPTIONAL)
    validate_attachments(payload.get("attachments"))
    validate_reply_to(payload.get("reply_to"))
...
def _format_message(event: _ValidatedEvent, room: DiscussionRoom) -> str:
    files = "".join(f"\nAttachment: {a['filename']} ({a['mime_type']}, artifact {a['artifact_id']})"
                    for a in event.payload.get("attachments", []))
    if event.kind == "message.user":
        return f"User (user): {event.payload['text']}{files}"
    return f"@{_member_by_id(room, event.payload['member_id']).handle}: {event.payload['text']}{files}"
...
def _build_prompt(
    *, room: DiscussionRoom, member: DiscussionMember, messages: Sequence[_ValidatedEvent], watermark: int,
    ...
    peers = ", ".join(f"@{candidate.handle}" for candidate in room.active_members if candidate.member_id != member.member_id)
    opening = [
...
def _make_task_plan(
    *, room: DiscussionRoom, discussion_event: _ValidatedEvent, member: DiscussionMember, member_index: int,
    ...
    if discussion_event.payload.get("attachments"):
        payload["attachments"] = list(discussion_event.payload["attachments"])
...
def plan_next_task(
    ...
    for round_index in range(MAX_DISCUSSION_ROUNDS):
        ...
        responders = (
            resolve_mentions((str(discussion.payload["text"]),), room.active_members) if round_index == 0
...
def _settled_effects(
    ...
    return {"message_event_id": message_event_id, "passed": False}, [EventPlan(
        ...
        payload={**_turn_coordinates(task), "text": text, **({"attachments": attachments} if attachments else {})},
...
def _result_attachments(result: Any) -> list[dict[str, Any]]:
    """Published-file references from a settled turn's result, or none.

    Validated on the way in, not merely copied: the result crosses a driver
    boundary and lands in a replayed log, so it is treated as input.
    """
    if not isinstance(result, Mapping):
        return []
    try:
        return validate_attachments(result.get("attachments"))
    except DiscussionValidationError:
        # A malformed reference must not cost the member their message.
        return []

def validate_attachments(value: Any) -> list[dict[str, Any]]:
    """Validate the published-file references carried on a room event.

    Field by field, and by shape: `artifact_id` must be the 32 lowercase hex the
    registry mints, because it is what the download route resolves. A room event
    is replayed and read by every participant, so a malformed or oversized
    reference is refused rather than stored and rendered.
    """
    if value is None:
        return []
    if not isinstance(value, list):
        raise DiscussionValidationError("attachments must be a list")
    if len(value) > MAX_EVENT_ATTACHMENTS:
        raise DiscussionValidationError("too many attachments")
    out: list[dict[str, Any]] = []
    for index, item in enumerate(value):
        entry = _exact_fields(
            item, label=f"attachment {index}", required=_ATTACHMENT_FIELDS
        )
        artifact_id = entry["artifact_id"]
        if not isinstance(artifact_id, str) or not _ARTIFACT_ID_RE.match(artifact_id):
            raise DiscussionValidationError(f"attachment {index} artifact_id is not valid")
        filename = entry["filename"]
        if not isinstance(filename, str) or not filename.strip():
            raise DiscussionValidationError(f"attachment {index} filename is required")
        mime_type = entry["mime_type"]
        if not isinstance(mime_type, str):
            raise DiscussionValidationError(f"attachment {index} mime_type must be a string")
        size_bytes = entry["size_bytes"]
        if not isinstance(size_bytes, int) or isinstance(size_bytes, bool) or size_bytes < 0:
            raise DiscussionValidationError(f"attachment {index} size_bytes must be a count")
        out.append({
            "artifact_id": artifact_id,
            # Bounded and stripped of separators for the same reason the
            # registry does it: this name is displayed, never resolved.
...
def validate_reply_to(value: Any) -> int | None:
    """Validate a reference to an earlier event in this room.

    A `seq` — the room log's own ordering key — rather than a free-form id, so a
    reference either names a real event in this room or is refused. The excerpt
    the client renders is resolved from that event server-side.
    """
    if value is None:
        return None
    if isinstance(value, bool) or not isinstance(value, int):
        raise DiscussionValidationError("reply_to must be an event seq")
    if value < 1:
        raise DiscussionValidationError("reply_to must be a positive event seq")
    return value
