...
def revoke_peer_reservations_for_profile(
    db_path: DbPath, *, target_profile: str, now: float | None = None) -> int:
    """Close the target-side fence on every live reservation aimed at one profile.

    An agent deletion must stop NEW peer work from being admitted at that target. A live
    reservation is exactly what admits it (``peer_room_grant_is_current``), so revoking
    every one at once is the whole fence — there is no second routing table to edit.
    Room membership and the append-only event log are deliberately untouched: the record
    of what the agent already did stays readable after it is gone.
    """
    profile = _actor_id(target_profile, "target_profile")
    timestamp = _now(now)
    with _transaction(db_path, immediate=True) as conn:
        return conn.execute(
            "UPDATE hosted_room_peer_reservations SET revoked_at=?, updated_at=?"
            " WHERE target_profile=? AND revoked_at IS NULL",
            (timestamp, timestamp, profile)).rowcount
...
def _reject_duplicate_identities(members: list[dict[str, Any]]) -> None:
    """Refuse a roster that names one identity twice.

    The same rule `hosted_room_discussion.validate_roster` applies, restated
    here because it must hold for the *merged* roster — the one that is
    actually stored — and this is the only place that exists.
    """
    for field, label in (
        ("member_id", "member ids"),
        ("profile", "member profiles"),
        ("handle", "member handles"),
    ):
        seen: set[str] = set()
        for member in members:
            ...
            if not value:
                continue
            if value in seen:
                raise ValueError(f"{label} must be unique, including removed members")
            seen.add(value)

def _retaining_removed_members(
    existing_json: str, proposed: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    """Carry anyone the caller left out forward as an inactive member.

    Deleting a member's entry would invalidate every event that names their
    ``member_id`` — which is the room's whole history from that member — so a
    removal is a flag, never a deletion. Enforced here, at the only layer that
    can see both the old roster and the new one, so no caller can drop a member
    by accident.
    """
    try:
    ...
    for entry in previous:
        if not isinstance(entry, dict):
            continue
        if str(entry.get("member_id")) in named:
            continue
        retired = dict(entry)
...
def set_room_members(db_path: DbPath, *, room_id: Any, event_id: Any, members: Any, now: float | None = None) -> dict[str, Any]:
    """Update membership atomically; retain removed identities for history replay."""
    room_id = _room_id(room_id)
    event_id = _event_id(event_id)
    proposed, _ = _validate_members(members)
    now = _now(now)
    actor_json = _system_actor_json("room-control")
    with _transaction(db_path, immediate=True) as conn:
        room = _room_row(conn, _SELECT_ROOM_WITH_BYTES, (room_id,), room_id)
        if room["disbanded_at"] is not None:
            raise RoomNotFoundError("hosted room not found")
        merged = _retaining_removed_members(room["members_json"], proposed)
        _reject_duplicate_identities(merged)
        members, members_json = _validate_members(merged)
        payload_json = _payload_json({"members": members})
        existing = _load_event(conn, room_id, event_id)
        if existing is not None:
        ...
        # Update roster and event under the same immediate transaction.
        conn.execute("""UPDATE hosted_rooms
                SET members_json=?, next_seq=?, event_bytes=event_bytes+?, revision=revision+1, updated_at=?
                WHERE room_id=?""", (members_json, seq + 1, event_bytes, now, room_id))
        conn.execute(_INSERT_EVENT, (
            room_id, seq, event_id, "room.members_changed", actor_json, int(room["authority_epoch"]), payload_json, now))
        updated = conn.execute(_SELECT_ROOM, (room_id,)).fetchone()
        return {**_room_from_row(updated), "event": _event_from_row(_load_event(conn, room_id, event_id))}
