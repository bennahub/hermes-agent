...
async def run_native_screencast(session: OwnerStreamSession, handle: RuntimeHandle, runtime: Any) -> None:
    """Private full-display capture, with the same public JSON/ACK contract."""
    from .native_desktop import NATIVE_LOCATION
    stop = threading.Event()
    session._stop_cdp = stop.set  # Existing session close hook; no CDP is opened.
    session.location = dict(NATIVE_LOCATION)
    while not session.closed and not stop.is_set():
        if session.broker.inflight >= session.broker.max_inflight:
            await asyncio.sleep(0.02)
            continue
        shot = await asyncio.to_thread(runtime.capture_frame, handle)
        if (shot["width"], shot["height"]) != (session.viewport_width, session.viewport_height):
            raise RuntimeError("native display geometry changed during stream")
        session.push_frame(session._seq + 1, shot["data"], shot["width"], shot["height"])
        await asyncio.sleep(0.08)
