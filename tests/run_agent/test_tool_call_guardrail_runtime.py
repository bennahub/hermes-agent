...
def test_default_run_conversation_warns_without_guardrail_halt(scoped_test_ingress):
    agent = _make_agent("web_search", max_iterations=10)
    same_args = {"query": "same"}
    scoped_test_ingress(agent)
    agent._test_execution_request("search repeatedly", [("web_search", same_args)])
    responses = [
        _mock_response(
            content="",
...
def test_guardrail_halt_emits_final_response_through_stream_delta_callback(scoped_test_ingress):
