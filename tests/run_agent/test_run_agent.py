...
@pytest.fixture()
def original_execution_request(agent, tmp_path, monkeypatch):
    """Use real ingress, scope ledger and admission with only offline policy responses."""
    ...
    def stage(instruction, tool, arguments):
        mint_execution_source(agent, instruction)

        def policy(**kwargs):
            assert kwargs["task"] == "execution_scope"
            payload = json.loads(kwargs["messages"][-1]["content"])
            if "original_instruction" in payload and "invocation" not in payload:
                assert payload["original_instruction"] == instruction
                decision = {"objective": instruction, "permitted": [instruction],
                            "excluded": ["Unrelated work"]}
            else:
                assert payload["original_instruction"] == instruction
                decision = {"allowed": payload["invocation"] == {"tool": tool, "arguments": arguments},
                            "reason": "Exact action for this test's original request"}
            return _mock_response(content=json.dumps(decision), finish_reason="stop")
...
class TestExecuteToolCalls:
    ...
    def test_keyboard_interrupt_emits_cancelled_post_tool_hook(self, agent, monkeypatch, original_execution_request):
        tc = _mock_tool_call(name="web_search", arguments='{"q":"test"}', call_id="c1")
        mock_msg = _mock_assistant_msg(content="", tool_calls=[tc])
        messages = []
        hook_calls = []
        ...
        original_execution_request("Search for test", "web_search", {"q": "test"})
        from agent.execution_scope import bind_agent_turn
        db = agent._session_db
        db.create_session(agent.session_id, "cli")
        row_id = db.append_message(agent.session_id, "user", "Search for test")
        bind_agent_turn(agent, "Search for test", {"_row_id": row_id})
...
class TestRunConversation:
    ...
    def test_tool_calls_then_stop(self, agent, original_execution_request):
        original_execution_request("search something", "web_search", {})
        self._setup_agent(agent)
        tc = _mock_tool_call(name="web_search", arguments="{}", call_id="c1")
    ...
    def test_repeated_scope_denial_stops_before_fourth_proposal(self, agent, original_execution_request):
        original_execution_request("search something", "web_search", {"q": "authorized"})
        self._setup_agent(agent)
        responses = [_mock_response(content="", finish_reason="tool_calls", tool_calls=[
            _mock_tool_call(name="web_search", arguments='{"q":"historical task"}', call_id=f"denied-{i}")
        ]) for i in range(4)]
        agent.client.chat.completions.create.side_effect = responses
        with patch("model_tools.handle_function_call") as dispatch, patch.object(agent, "_save_trajectory"), patch.object(agent, "_cleanup_task_resources"):
            result = agent.run_conversation("search something")
        assert result["failed"] is True
        assert result["turn_exit_reason"] == "execution_scope_denied"
        assert result["api_calls"] == 3
        dispatch.assert_not_called()
        assert getattr(agent, "_owner_continuity_work_id", None) is None
    ...
    def test_truncated_tool_call_retries_once_before_refusing(self, agent, original_execution_request):
        ...
        proceeds."""
        original_execution_request(
            "write the report", "write_file", {"path": "report.md", "content": "full content"},
        )
        self._setup_agent(agent)
    ...
    def test_stub_stall_mid_tool_call_recovers_within_3_retries(self, agent, original_execution_request):
    ...
    def test_zero_byte_tool_args_stub_recovers_within_retries(self, agent, original_execution_request):
...
def test_execution_scope_denial_preserves_native_effect_state(
    agent, monkeypatch, original_execution_request, admitted
):
    from agent.execution_scope import bind_agent_turn, get_binding, close_turn
    agent.session_id = "scope-effect-session"
    agent._current_turn_id = "scope-effect-turn"
    agent._current_api_request_id = "scope-effect-api"
    original_execution_request("Search for test", "web_search", {"q": "test"})
    db = agent._session_db
    db.create_session(agent.session_id, "cli")
    row_id = db.append_message(agent.session_id, "user", "Search for test")
    bind_agent_turn(agent, "Search for test", {"_row_id": row_id})
    binding = get_binding(agent._current_turn_id)
    args = {"q": "test" if admitted else "unrelated historical task"}
    if admitted:
        claim = db.claim_scope_action(binding.scope_id, "turn:scope-effect-turn:call:effect-call", "web_search", args)
        assert claim["status"] == "claimed"
    messages = []
    tc = _mock_tool_call(name="web_search", arguments=json.dumps(args), call_id="effect-call")
    try:
        with patch("model_tools.handle_function_call") as handler:
            agent._execute_tool_calls_sequential(_mock_assistant_msg(content="", tool_calls=[tc]), messages, "scope-task")
            handler.assert_not_called()
        row = messages[-1]
        assert row["effect_disposition"] == ("unknown" if admitted else "none")
        # The production web-tool envelope labels its payload untrusted.
        assert ("execution_scope_uncertain" if admitted else "execution_scope_denied") in row["content"]
        assert '"retryable": false' in row["content"]
        assert getattr(agent, "_owner_continuity_work_id", None) is None
    finally:
        close_turn(agent)


def test_policy_worker_timeout_is_not_external_uncertainty(agent, monkeypatch, original_execution_request):
    import threading
    from agent.execution_scope import bind_agent_turn, get_binding, close_turn
    from agent import tool_executor
    agent.session_id = "policy-timeout-session"
    agent._current_turn_id = "policy-timeout-turn"
    original_execution_request("Search for test", "web_search", {"q": "test"})
    db = agent._session_db
    db.create_session(agent.session_id, "cli")
    row_id = db.append_message(agent.session_id, "user", "Search for test")
    bind_agent_turn(agent, "Search for test", {"_row_id": row_id})
    release, policy_done, policy_entered = threading.Event(), threading.Event(), threading.Event()
    def slow_policy(*args, **kwargs):
        policy_entered.set()
        assert release.wait(5)
        policy_done.set()
        return True, "Current instruction"
    monkeypatch.setattr("agent.execution_scope.judge_action", slow_policy)
    monkeypatch.setattr(tool_executor, "_resolve_sequential_tool_timeout", lambda: 0.05)
    poll = tool_executor._poll_sequential_future
    def poll_after_policy_started(agent, future, function_name, deadline, started, gate):
        import time
        assert policy_entered.wait(5)
        return poll(agent, future, function_name, time.monotonic() + 0.05, started, gate)
    monkeypatch.setattr(tool_executor, "_poll_sequential_future", poll_after_policy_started)
    tc = _mock_tool_call(name="web_search", arguments='{"q":"test"}', call_id="slow-policy")
    messages = []
    try:
        with patch("model_tools.handle_function_call") as handler:
            agent._execute_tool_calls_sequential(_mock_assistant_msg(content="", tool_calls=[tc]), messages, "policy-timeout-task")
            assert messages[-1]["effect_disposition"] == "none"
            assert agent._execution_scope_stop == "execution_scope_unavailable"
            binding = get_binding(agent._current_turn_id)
            assert binding.revoked
            release.set()
            assert policy_done.wait(5)
            handler.assert_not_called()
            assert db.get_scope_action(binding.scope_id, "turn:policy-timeout-turn:call:slow-policy") is None
    finally:
        release.set()
        close_turn(agent)
