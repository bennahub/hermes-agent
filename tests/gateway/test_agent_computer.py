...
def _svc(
    ...
    return AgentComputerService(
        ...
        takeover_ttl_s=60,
        max_active_computers=max_active_computers,
    )
...
def test_wake_reclaims_stale_slot_then_starts(tmp_path):
    clock = Clock()
    svc = _svc(tmp_path, clock=clock, max_active_computers=2)
    first = svc.ensure_computer("idle-a")
    second = svc.ensure_computer("idle-b")
    third = svc.ensure_computer("need-slot")
    svc.wake(first.id, agent_principal("idle-a"))
    svc.wake(second.id, agent_principal("idle-b"))
    clock.advance(31 * 60)
    computer, _lease = svc.wake(third.id, agent_principal("need-slot"))
    assert computer.lifecycle == Lifecycle.READY
    slept = [
        item.id
        for item in (svc.get_computer(first.id), svc.get_computer(second.id))
        if item.lifecycle == Lifecycle.SLEEPING
    ]
    assert slept
    assert svc.get_computer(third.id).lifecycle == Lifecycle.READY


def test_wake_does_not_reclaim_fresh_active_computer(tmp_path):
    clock = Clock()
    svc = _svc(tmp_path, clock=clock, max_active_computers=1)
    active = svc.ensure_computer("live-work")
    waiting = svc.ensure_computer("waiting")
    svc.wake(active.id, agent_principal("live-work"))
    clock.advance(10)
    with pytest.raises(ComputerCapacityError):
        svc.wake(waiting.id, agent_principal("waiting"))
    assert svc.get_computer(active.id).lifecycle == Lifecycle.READY
    assert svc.store.active_lease_for_computer(waiting.id) is None
