...
@pytest.mark.asyncio
async def test_run_endpoint_freezes_current_input_separately_from_history(monkeypatch):
    ...
    agent=MagicMock()
    agent.session_id="run-source-test"
    agent.session_prompt_tokens=agent.session_completion_tokens=agent.session_total_tokens=0
    observed=[]
    ready=threading.Event()
    def run(**kwargs):
        observed.append(consume_execution_source(agent))
        ready.set()
    ...
    with patch.object(adapter,"_create_agent",return_value=agent):
        async with TestClient(TestServer(_create_runs_app(adapter))) as client:
            response=await client.post("/v1/runs",json={"input":"inspect new report","conversation_history":[{"role":"user","content":"old command"}]})
            assert response.status==202, await response.text()
            assert await asyncio.to_thread(ready.wait,3)
    assert observed[0]["instruction"]=="inspect new report"
    assert observed[0]["source"]=="api_run"
