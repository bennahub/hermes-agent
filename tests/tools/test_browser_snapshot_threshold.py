...
def _write_threshold(hermes_home, value):
    ...
    # Rapid same-size writes may share an mtime on this filesystem. The raw
    # config cache intentionally keys on mtime/size; represent a distinct edit.
    current = path.stat()
    if previous is not None and current.st_mtime_ns == previous:
        os.utime(path, ns=(current.st_atime_ns, previous + 1))
