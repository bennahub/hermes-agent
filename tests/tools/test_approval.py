...
class TestSmartApproval:
    def test_smart_approval_is_deterministic_without_guardian_llm(self):
        with mock_patch("agent.auxiliary_client.call_llm") as mock_call:
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        # The exemption compares against the CANONICAL temp directory, so a
        # hostile symlink swap cannot launder an `rm` past the guard (the rule
        # the sibling symlink test pins). This test used to hard-code "/tmp",
        # which is itself a symlink on macOS — so it asserted that a path the
        # guard deliberately refuses to exempt would be exempt, and failed
        # there while passing on Linux. Canonicalise first and it exercises the
        # exemption it was written for, on either platform.
