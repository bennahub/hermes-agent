...
class TestRunJobWakeGate:
    ...
    def _make_job(self, script_output):
        from cron.jobs import create_job
        from hermes_constants import get_hermes_home

        home = get_hermes_home()
        (home / "scripts").mkdir(parents=True, exist_ok=True)
        (home / "scripts" / "check.py").write_text(f"print({script_output!r})\n")
        return home, create_job("Do a thing", "every 5m", script="check.py")

    def test_wake_false_skips_agent_and_returns_silent(
        self, caplog, migrate_configured_cron_job, run_scoped_cron_job,
    ):
        from cron.scheduler import SILENT_MARKER

        home, job = self._make_job('{"wakeAgent": false}')
        job = migrate_configured_cron_job(home, job)
        with patch("run_agent.AIAgent") as agent_cls:
            success, doc, final, err = run_scoped_cron_job(job)
        assert success is True
        assert err is None
    ...
    def test_wake_true_runs_agent_with_injected_output(
        self, migrate_configured_cron_job, run_scoped_cron_job,
    ):
        script_output = '{"wakeAgent": true, "data": {"new": 3}}'
        home, job = self._make_job(script_output)
        job = migrate_configured_cron_job(home, job)
        agent = MagicMock()
        agent.run_conversation = MagicMock(return_value={
            "final_response": "ok", "messages": [],
        })
        with patch("run_agent.AIAgent", return_value=agent) as agent_cls:
            success, doc, final, err = run_scoped_cron_job(job)
