...
class HermesACPAgent(SlashCommandsMixin, acp.Agent):
    ...
    def _claim_turn_or_queue(
        ...
        *, original_instruction: Any = None, original_prompt: Any = None,
    ) -> str | None:
        ...
        with state.runtime_lock:
            ...
            if text_only and isinstance(user_content, str) and hasattr(state.agent, "redirect") and (
                ...
                try:
                    ...
                    if admit_owner_amendment(state.agent, original_instruction or user_content,
                                             state.agent.redirect):
            ...
            state.queued_prompts.append(
                original_instruction if isinstance(original_instruction, str)
                else {"prompt": original_prompt, "preview": user_text or "[Image attachment]"}
                if original_prompt is not None else user_text or "[Image attachment]"
            )
        ...
        with contextlib.ExitStack() as stack:
            ...
            try:
                if original_instruction is not None:
                    ...
                    mint_execution_source(agent, original_instruction, source="acp.prompt")
    ...
    async def prompt(self, prompt: list[PromptBlock], session_id: str, **kwargs: Any) -> PromptResponse:
        ...
        # Preserve this native request before interrupted-history enrichment.
        import copy

        original_instruction = copy.deepcopy(user_content)
        ...
        absorbed = self._claim_turn_or_queue(
            ...
            original_instruction=original_instruction, original_prompt=copy.deepcopy(prompt),
        )
    ...
    async def _finish_turn(
        ...
        while True:
            ...
            queued_blocks = (next_prompt["prompt"] if isinstance(next_prompt, dict)
                             else [TextContentBlock(type="text", text=next_prompt)])
            preview = next_prompt["preview"] if isinstance(next_prompt, dict) else next_prompt
            if conn:
                await conn.session_update(session_id, acp.update_user_message_text(preview))
            await self.prompt(prompt=queued_blocks, session_id=session_id)
