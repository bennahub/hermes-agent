...
class SessionGatewayMixin:
    ...
    def record_gateway_session_peer(
        ...
        origin_json: str = None, hidden: bool = False,
        include_compression_ancestors: bool = False) -> None:
            ...
            if conn.execute("SELECT 1 FROM sessions WHERE id = ? LIMIT 1", (session_id,)).fetchone() is None:
                conn.execute(
                    """INSERT INTO sessions (
                               ...
                               profile_name, started_at, hidden
                           )
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                           ON CONFLICT(id) DO UPDATE SET
                               ...
                               hidden = CASE WHEN excluded.hidden = 1 THEN 1 ELSE sessions.hidden END""",
                    ...
                    (session_id, source, user_id, session_key, chat_id, chat_type, thread_id, display_name,
                     origin_json, self._own_profile_name(), time.time(), 1 if hidden else 0),
                )
