...
def release_all_under(directory: "str | Path") -> int:
    """Force-close every shared SessionDB whose file lives under *directory*; returns the count.

    ``release`` is refcount-driven, so a live holder keeps a profile's ``state.db`` open — and
    ``profiles.delete`` served by a gateway is exactly that case: ``compute_host`` runs another
    profile's turn INSIDE the gateway process (``set_hermes_home_override`` + ``acquire`` on
    that home's ``state.db``), so the PID sweeps in ``hermes_cli.profiles`` cannot see the
    holder — it is the deleting process itself. Left open, the handle takes writes through the
    ``rmtree`` on POSIX and fails the ``rmtree`` outright on Windows (``WinError 32``), which
    leaves the profile tombstoned but present.

    The directory is going away, so later use by a stale holder is *expected* to fail: it
    raises rather than resurrecting a deleted profile's rows. Retired generations under the
    directory go too — their holders' releases become no-ops, which is the same contract
    ``close_all`` already has. In a process holding none (the CLI deleting from outside a
    gateway) this is a harmless no-op returning 0. Mirrors
    ``plugins.memory.holographic.store.MemoryStore.release_all_under``, deliberately: the two
    handle kinds that survive a profile delete should be released the same way.
    """
    ...
    try:
        ...
        pass
    root = os.path.normcase(str(raw)) + os.sep

    def _under(path: Optional[Path]) -> bool:
        return path is not None and os.path.normcase(str(path)).startswith(root)

    with _lock:
        doomed = [_generations.pop(path) for path in list(_generations) if _under(path)]
        doomed += [_retired.pop(key) for key, gen in list(_retired.items())
                   if _under(_db_path_of(gen.db))]
        for generation in doomed:
            generation.retired = True
    # Teardown OUTSIDE the lock, for the same reason ``release`` does it.
    for generation in doomed:
