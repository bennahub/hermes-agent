...
# DeepSeek V4.1 Flash launched on 2026-09-10 with ``deepseek-flash`` as the
# canonical API id. ``deepseek-v4-flash`` remains a temporary compatibility
# route, while the older chat/reasoner aliases are retired. Preserve explicit
# V-series ids but normalize fuzzy/retired names to the durable canonical id.
...
def _normalize_for_deepseek(model_name: str) -> str:
    """Map a model input to a DeepSeek-accepted id.

    Canonicals and explicit ``deepseek-v<digit>…`` ids pass through. Retired
    aliases and fuzzy names resolve to V4.1 Flash's canonical ``deepseek-flash``.
    """
