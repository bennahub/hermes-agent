...
from gateway import path_authorization
...
_HOSTED_MANAGED_FILES_ROOT = path_authorization.HOSTED_MANAGED_FILES_ROOT
...
def _resolve_managed_path(
    ...
    requested_profile = _path_text(query.get("profile")) if query is not None else None
    _enforce_requested_profile(requested_profile, resolved)
...
def _enforce_requested_profile(requested_profile: str | None, resolved: Path) -> None:
    """Confine `resolved` to a `profile` the caller named, or reject it.

    The value is validated, never trusted: a bad or reserved profile is rejected
    (400) rather than allowed to fall through and leave the check off, and
    `default` may not reach a NAMED profile's subtree — that would be the same
    cross-profile read spelled differently. A named profile confines the target
    to its own directory after symlink canonicalisation.
    """
    if not requested_profile:
        return
    # Validate with Hermes' own canonical rules — the same normalize + validate
    # the rest of the system uses, so reserved names (`root`, `hermes`, `test`),
    # dotted, and mixed-case forms are handled identically here and everywhere
    # else, rather than by a second, looser regex that would drift.
    from hermes_cli.profiles import (
        get_profile_dir, _get_profiles_root, normalize_profile_name, validate_profile_name,
    )
    try:
    ...
    try:
        profiles_root = _canonical_path(Path(_get_profiles_root()))
    except Exception:
        profiles_root = None
    if requested_profile == "default":
        if profiles_root is not None and _path_is_under(profiles_root, resolved):
            raise HTTPException(status_code=403, detail="Path outside the requested profile")
        return
    try:
        profile_root = _canonical_path(Path(get_profile_dir(requested_profile)))
    except Exception:
        raise HTTPException(status_code=400, detail="Unknown profile")
    # The named profile's directory must sit under the profiles root (guards
    # against get_profile_dir returning something unexpected), and the target
    # must resolve inside it.
    if (profiles_root is None
            or not _path_is_under(profiles_root, profile_root)
            or not _path_is_under(profile_root, resolved)):
        raise HTTPException(status_code=403, detail="Path outside the requested profile")


def _enforce_read_allowlist(policy: ManagedFilesPolicy, target: Path) -> None:
    """Refuse a read outside the allowed trees when no root is configured.

    The judgement is shared with artifact publication; only the refusal is
    local, because a 403 is this surface's vocabulary and ``PublishRefused`` is
    the other's.
    """
    if policy.locked_root is not None:
        return  # an explicit root already applied the stricter check
    if path_authorization.is_within_allowed_roots(target):
        return
    raise HTTPException(status_code=403, detail="Path outside managed files root")
