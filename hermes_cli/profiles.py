...
def named_profile_dirs_in(profiles_root: Path, *, live_only: bool = True) -> List[Path]:
    """Sorted named-profile dirs under an EXPLICIT profiles root; ``live_only`` skips tombstones.

    The rule for "is this directory entry a profile" lives here, once. A gateway serving a
    sibling profile resolves its own root (from the active ``HERMES_HOME`` override, which need
    not be this process's default), so it needs the rule without the root — and a second copy of
    the rule is what let the change watcher count a tombstoned name as a live profile."""
    try:
        entries = sorted(Path(profiles_root).iterdir())
    except OSError:
        return []
    return [
...
def _iter_named_profile_dirs(*, live_only: bool = True) -> List[Path]:
    ...
    return named_profile_dirs_in(_get_profiles_root(), live_only=live_only)


# Per-name reentrancy state for _profile_name_lock. Keyed by name so holding the lock for one
# profile never satisfies the depth guard for another.
_profile_name_lock_holders: Dict[str, threading.local] = {}
_profile_name_lock_holders_guard = threading.Lock()


def _profile_name_lock_holder(canon: str) -> threading.local:
    with _profile_name_lock_holders_guard:
        return _profile_name_lock_holders.setdefault(canon, threading.local())


@contextlib.contextmanager
def _profile_name_lock(canon: str, *, timeout: float = 30.0):
    """Serialise create and delete of ONE profile name, across processes.

    ``delete_profile`` tombstones then ``rmtree``s; ``create_profile`` clears the tombstone then
    bootstraps. Interleaved, a create landing between those two steps has its tombstone cleared
    and its fresh files walked by the delete's ``rmtree`` — and BOTH calls return ok, so the
    Owner is told a profile was created that is not there. The window is narrow and the loss is
    total, so both halves take this lock.

    Degrades to a no-op (with a warning) when the host has no kernel lock primitive or the lock
    file cannot be written: the pre-existing unguarded behaviour is better than refusing to
    delete a profile because a lock directory is read-only. A ``TimeoutError`` propagates —
    waiting 30s and then racing anyway would defeat the point.

    The lock files live at the hermes ROOT, deliberately NOT inside ``profiles/``: several
    callers enumerate that directory with a bare ``is_dir()`` and would each have to learn
    about a new entry there."""
    stack = contextlib.ExitStack()
    with stack:
        try:
            from hermes_cli import auth
            if auth.fcntl is not None or auth.msvcrt is not None:
                stack.enter_context(auth._file_lock(
                    _get_default_hermes_home() / ".profile-locks" / f"{canon}.lock",
                    _profile_name_lock_holder(canon), timeout,
                    f"another create or delete of profile '{canon}' is already running"))
        except TimeoutError:
            raise
        except Exception as exc:
            print(f"⚠ Could not take the profile-name lock for '{canon}' ({exc}); a concurrent "
                  f"create or delete of the same name is not serialised.")
        yield


@contextlib.contextmanager
def _profile_delete_lock(canon: str):
    """``_profile_name_lock`` held across the WHOLE delete, with an actionable timeout.

    Taken at the tombstone — where the create/delete race actually bites — the 30 s timeout
    fired *after* ``_cleanup_gateway_service``, ``_maybe_unregister_gateway_service``,
    ``_stop_gateway_process`` and ``_stop_profile_backends`` had already run: the service was
    left disabled and the gateway stopped while the profile itself survived, reported as a
    bare ``TimeoutError``. Acquiring first makes the timeout free — no step has run — and the
    message says so, the way the ``rmtree`` failure path names its state and its remedy.

    Only the acquisition is rewritten; a ``TimeoutError`` raised by the delete itself keeps
    its own message. The lock is deliberately NOT held across the confirmation prompt: a
    cross-process lock must never wait on a human.
    """
    with contextlib.ExitStack() as stack:
        try:
            stack.enter_context(_profile_name_lock(canon))
        except TimeoutError as exc:
            raise TimeoutError(
                f"{exc}. Nothing about '{canon}' was changed — its gateway, service, alias and "
                f"files are exactly as they were. Wait for that operation to finish, then run "
                f"the delete again."
            ) from exc
        yield
...
def _seed_model_config(profile_dir: Path, *, inherit_credentials: bool = True) -> None:
    ...
    with contextlib.suppress(Exception):  # creation must not fail over this; `hermes model` sets it later
        import yaml
        from hermes_constants import get_hermes_home
        from hermes_cli.config import clear_model_endpoint_credentials, read_user_config_raw
        source = get_hermes_home() / "config.yaml"
        model_cfg = read_user_config_raw(source).get("model") if source.is_file() else None
        if model_cfg:
            if not inherit_credentials and isinstance(model_cfg, dict):
...
def _bootstrap_profile_dir(profile_dir: Path, source_dir: Optional[Path], *, inherit_model_credentials: bool = True) -> None:
    ...
    if source_dir is None:
        _seed_model_config(profile_dir, inherit_credentials=inherit_model_credentials)
        return
...
def create_profile(
    ...
    inherit_model_credentials: bool = True,
) -> Path:
    ...
    profile_dir = get_profile_dir(canon)
    # Existence check → tombstone clear → bootstrap is one indivisible act: a delete of the same
    # name that interleaves here clears nothing of its own but rmtree's what this call just
    # wrote, and both calls return ok. See _profile_name_lock.
    with _profile_name_lock(canon):
        ...
        if clone_all and source_dir:
            ...
            _bootstrap_profile_dir(profile_dir, source_dir, inherit_model_credentials=inherit_model_credentials)
...
def _retire_shared_agent_resources(canon: str, profile_dir: Path) -> None:
    """Retire the parts of a deleted agent that do NOT live under its profile directory.

    ``rmtree`` reaches everything in ``profiles/<name>``. It cannot reach the four places a
    permanent agent also exists, all rooted at the OWNER's home:

      * ``state.db`` — hosted-room peer reservations. A live reservation is the fence that
        admits new A2A/RoomLink work at a target, so one left behind keeps a deleted agent
        routable.
      * ``agent-computers/state.db`` — its exclusive logical Computer. One left behind keeps
        a machine with no agent behind it in the Owner's computer list.
      * ``a2a_threads.db`` — the peer-message ledger. A thread keys on the agent's NAME, and a
        name is a slug the Owner can mint again, so a recreated ``badr`` addressed the deleted
        ``badr``'s conversations. Retiring the name floors what the next holder of that slug can
        reach, without touching one append-only row. Same reasoning as the browser identity the
        computer retirement releases, applied to the other thing a slug re-inherits.
      * ``state/connection_health.json`` — recorded Connection faults, keyed ``<scope>/<id>``. An
        MCP fault is recorded under the serving profile, and the only clearer is a successful call
        running AS that profile: left behind, the row is one no Owner can ever act on, and the
        respawn guard holds autonomous work on it forever.

    Best-effort by design, like the memory-store release in ``delete_profile``: a checkout
    without the gateway installed, or a store that will not open, must never strand a
    half-deleted profile. Historical evidence is preserved on purpose — room membership, the
    append-only event log and the A2A ledger's own rows are untouched, and the computer's audit
    rows outlive its row.

    Best-effort is NOT silent. A missing gateway is expected and says nothing; anything else
    is reported with the consequence named, because the failure that matters here —
    ``SQLITE_BUSY`` on the shared root ``state.db``, which the revoke opens ``IMMEDIATE`` —
    leaves the A2A routing fence OPEN, the one thing this step exists to close. It was
    printing on success and nothing at all on failure.

    This runs FIRST, before the gateway is stopped, so the routing fence closes while the runtime
    is still reachable. That leaves a teardown-sized window in which the dying gateway could record
    one more fault under the name just retired. Neither of the last two steps is the safety net for
    that, and neither needs to be: ``connection_health._live_faults`` ignores a fault whose scope
    names no live profile wherever one is read, so a row written into that window is inert whether
    or not this pass saw it. What happens here is hygiene — keeping the store from accumulating
    rows addressed to agents that are gone.
    """
    root = profile_dir.parent.parent if profile_dir.parent.name == "profiles" else profile_dir
    revoked = _best_effort_retirement_step(
        "revoke peer-room reservations",
        f"'{canon}' may still be admitted as an A2A/RoomLink target",
        lambda: _revoke_peer_reservations_with_retry(root / "state.db", canon),
    )
    if revoked:
        print(f"✓ Revoked {revoked} peer-room reservation(s) targeting '{canon}'")
    retired = _best_effort_retirement_step(
        "retire the agent computer",
        f"a machine with no agent behind it may remain in the computer list for '{canon}'",
        lambda: _retire_agent_computer(canon),
    )
    if retired and retired.get("retired"):
        print(f"✓ Retired agent computer {retired.get('computer_id')}")
    _best_effort_retirement_step(
        "retire the agent's A2A message history",
        f"a profile recreated as '{canon}' would inherit the deleted agent's conversations",
        lambda: _retire_a2a_history(root, canon),
    )
    _best_effort_retirement_step(
        "clear recorded Connection faults",
        f"a fault recorded by '{canon}' may linger in the health store",
        lambda: _forget_connection_faults(canon),
    )


def _retire_a2a_history(root: Path, canon: str) -> float:
    """Floor the A2A ledger for *canon*: nothing recorded before now is this name's any more."""
    from gateway.a2a_threads import retire_agent
    retired_at = retire_agent(root, canon)
    if retired_at:
        print(f"✓ Retired '{canon}'s A2A message history (kept in the ledger, unaddressable)")
    return retired_at


def _forget_connection_faults(canon: str) -> int:
    from agent.connection_health import forget_scope
    dropped = forget_scope(canon)
    if dropped:
        print(f"✓ Cleared {dropped} recorded Connection fault(s) scoped to '{canon}'")
    return dropped


def _best_effort_retirement_step(what: str, consequence: str, run):
    """Run one retirement step; never raise. ``None`` when it did not complete.

    An absent gateway package is the documented no-op and stays quiet. Every other failure is
    printed with the consequence spelled out — a silently skipped step here is a fence left
    open, which is indistinguishable from success to every caller."""
    try:
        return run()
    except (ImportError, ModuleNotFoundError):
        return None  # gateway not installed in this checkout — the documented no-op
    except Exception as exc:
        print(f"⚠ Could not {what}: {exc}\n  → {consequence}.")
        return None


def _revoke_peer_reservations_with_retry(db_path: Path, canon: str, *, attempts: int = 4) -> int:
    """Revoke every reservation targeting *canon*, retrying a locked root ``state.db``.

    The root store is shared with every other profile's gateway, and the revoke runs
    ``BEGIN IMMEDIATE``, so a concurrent writer is a normal transient — not a reason to leave
    the fence open. Only lock/busy errors are retried; anything else raises straight to the
    caller, which reports it."""
    import sqlite3

    from gateway.hosted_rooms import revoke_peer_reservations_for_profile

    for attempt in range(attempts):
        try:
            return revoke_peer_reservations_for_profile(db_path, target_profile=canon)
        except sqlite3.OperationalError as exc:
            if attempt == attempts - 1 or not any(
                    marker in str(exc).lower() for marker in ("locked", "busy")):
                raise
            time.sleep(0.25 * (attempt + 1))
    return 0  # unreachable: the final attempt either returns or raises


def _retire_agent_computer(canon: str) -> dict:
    from gateway.agent_computer import get_contract
    from gateway.agent_computer.models import OWNER_PRINCIPAL
    return get_contract().retire_profile(canon, OWNER_PRINCIPAL)


def _stop_in_process_profile_work(canon: str, profile_dir: Path) -> None:
    """Stop the work this process is doing FOR another profile before its tree is removed.

    ``_stop_gateway_process`` knows ``gateway.pid`` and ``_stop_profile_backends`` sweeps other
    PIDs — and both deliberately skip this process and its ancestors. But a gateway runs another
    profile's turn INSIDE itself (``tui_gateway.compute_host`` binds ``HERMES_HOME`` and acquires
    that home's ``state.db``), and that gateway is the very process servicing ``profiles.delete``.
    So the one holder nothing could see was us.

    Left running: on POSIX the tree is removed anyway and the turn fails on its next write, and
    on Windows the open handle fails ``rmtree`` outright, leaving the profile tombstoned but
    present — invisible in the roster with its data on disk. Both are silent."""
    interrupted = _interrupt_live_profile_turns(profile_dir)
    if interrupted:
        print(f"✓ Interrupted {interrupted} in-flight turn(s) bound to '{canon}'")
    try:
        from hermes_state_registry import release_all_under
        released = release_all_under(profile_dir)
    except Exception as exc:
        print(f"⚠ Could not release session-database handles for '{canon}': {exc}\n"
              f"  → removing {profile_dir} may fail, or a stale handle may keep writing to it.")
        return
    if released:
        print(f"✓ Released {released} session-database handle(s) held by this process")


def _interrupt_live_profile_turns(profile_dir: Path, *, wait: float = 5.0) -> int:
    """Cancel every live in-process turn bound to *profile_dir*; return how many. 0 outside a
    gateway process. The wait is bounded: a turn wedged in a blocked tool must delay the delete,
    not own it — the tombstone and the handle release still follow."""
    # Looked up in ``sys.modules``, never imported: only a process that ALREADY loaded the
    # gateway server can be running a turn, and a CLI delete must not drag the whole server
    # module (and its import-time side effects) in just to find nothing.
    gw_server = sys.modules.get("tui_gateway.server")
    if gw_server is None:
        return 0  # not a gateway process (CLI delete): nothing in-process to stop
    interrupt = getattr(gw_server, "_interrupt_session_turn", None)
    if not callable(interrupt):
        return 0
    homes = {str(profile_dir)}
    with contextlib.suppress(OSError):
        homes.add(str(profile_dir.resolve()))
    try:
        with gw_server._sessions_lock:
            claimed = [(sid, sess) for sid, sess in list(gw_server._sessions.items())
                       if str((sess or {}).get("profile_home") or "") in homes]
    except Exception:
        return 0
    running = [(sid, sess) for sid, sess in claimed if sess.get("running")]
    for sid, session in running:
        with contextlib.suppress(Exception):
            interrupt(sid, session)
    deadline = time.monotonic() + max(0.0, wait)
    while running and time.monotonic() < deadline:
        if not any(sess.get("running") for _, sess in running):
            break
        time.sleep(0.1)
    still_running = [sid for sid, sess in running if sess.get("running")]
    if still_running:
        print(f"⚠ {len(still_running)} turn(s) for this profile did not stop within {wait:g}s; "
              f"their next write will fail rather than resurrect the deleted profile.")
    return len(running)
...
def _print_delete_summary(canon: str, profile_dir: Path, gw_running: bool, wrapper_path: Optional[Path]) -> None:
    ...
    # The two things that do NOT live under profiles/<name> — the summary is the only place
    # the Owner is told what a delete reaches, so it has to name them or it is a lie.
    print("  • Its agent computer, including the private workspace and downloads it kept")
    print("  • Its peer-room reservations (it stops being reachable as an A2A target)")
    if wrapper_path is not None:
        print(f"  • Command alias ({wrapper_path})")
    print("\nThis will be KEPT:")
    print("  • Room membership, the room event log, and the computer's audit trail")
    # Kept AND unreachable is a third state, so saying only "kept" would read as "a new agent
    # with this name picks the conversations back up" — which is the defect the retirement
    # closed — and saying only "deleted" would be false about an append-only ledger.
    print("  • Its messages to and from other agents — kept for the record, but an agent")
    print("    created with this name later cannot open or continue them")
    print("  • Any browser identity it shared with another agent (released, not destroyed)")
    print("  • A browser identity only it owned — revoked, but its stored browser profile,")
    print("    cookies included, stays on disk")
    if gw_running:
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    # Everything below is destructive. The name lock is taken FIRST, not at the tombstone:
    # taken there, a lock timeout aborted with the service already disabled and the gateway
    # already stopped while the profile survived. See _profile_delete_lock.
    with _profile_delete_lock(canon):
        # 0. Close routing FIRST: revoke the A2A peer-room fence and retire the logical Computer
        # while the runtime is still reachable. Doing this after the gateway is stopped would leave
        # a window where new peer work is still admitted at a target that is being torn down.
        _retire_shared_agent_resources(canon, profile_dir)
        ...
        # Tombstone → rmtree is one indivisible act inside that hold: a concurrent create of
        # the same name clears the tombstone and writes files this rmtree would then walk.
        ...
        # 2d. Stop the work THIS process is doing for the profile. The PID sweeps above skip
        # ourselves and our ancestors on purpose, and a gateway runs another profile's turn
        # inside itself — so this is the one holder nothing else can reach. Runs after the
        # tombstone, so a straggler write that reaches the divert path is refused instead of
        # writing a fragment of the deleted conversation back under the deleted home.
        _stop_in_process_profile_work(canon, profile_dir)
        ...
        try:
            ...
            # Bounded and named, not silent: the profile is now tombstoned but present — gone
            # from the roster with its data on disk, and `profile create` will refuse the name
            # while config.yaml/.env survive. Say so, here, where the operator can act.
            print(f"⚠ Could not remove {profile_dir}: {e}")
            print(f"  → '{canon}' is now hidden from the roster but its files are still on disk, "
                  f"and creating a profile named '{canon}' will be refused until they are gone. "
                  f"Close anything still using it and remove {profile_dir} by hand.")
            remove_error = e
    ...
    if remove_error is not None:
        raise RuntimeError(
            f"Could not remove profile directory {profile_dir}: {remove_error}. '{canon}' is "
            f"tombstoned but still on disk; remove {profile_dir} by hand before reusing the name."
        ) from remove_error
