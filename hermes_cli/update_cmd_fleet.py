...
def _live_gateway_code_sha() -> str | None:
    """SHA the hosted gateway stamped into ``gateway_state.json``, if any."""
    try:
        from hermes_constants import get_default_hermes_root

        payload = json.loads((get_default_hermes_root() / "gateway_state.json").read_text(encoding="utf-8"))
    except Exception:
        return None
    sha = payload.get("code_sha") if isinstance(payload, dict) else None
    return str(sha) if sha else None
...
def _receipt_reports_stale_runtime(expected_sha: str | None = None) -> bool:
    ...
    *before* the pull, so a finished update's plan always looks stale and must not
    ...
    A leftover receipt must not contradict the hosted runtime: when the live
    gateway SHA already matches this process identity, the fleet is current.
    ...
    if not expected_sha:
        return False
    live_sha = _live_gateway_code_sha()
    if live_sha and str(live_sha) == str(expected_sha):
        return False
