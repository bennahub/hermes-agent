...
class DispatchResult:
    ...
    """Task ids whose workers stopped on a Connection only the Owner can restore
    (``KANBAN_NEEDS_OWNER_EXIT_CODE``). Also released without counting a failure,
    but reported separately: calling an Owner-gated fault a rate limit is exactly
    the kind of near-miss wording this taxonomy exists to remove, and unlike a
    quota window it does not clear on its own."""
...
def _classify_worker_exit(pid: int) -> "tuple[str, Optional[int]]":
    ...
    (``KANBAN_RATE_LIMIT_EXIT_CODE``, never counts as a failure), ``needs_owner``
    (``KANBAN_NEEDS_OWNER_EXIT_CODE``, also no failure but held against the fault
    rather than a clock), ``nonzero_exit``, ``signaled`` (``code`` is the signal),
    ``unknown`` (pid not in the reap registry; ``code`` None)."""
    ...
    try:
        if os.WIFEXITED(raw):
            ...
            if code == _kb.KANBAN_NEEDS_OWNER_EXIT_CODE:
                return ("needs_owner", code)
...
class _DeadWorker:
    ...
    @property
    def released_without_failure(self) -> bool:
        """Requeued, not failed: the attempt hit a wall no retry budget should be spent on."""
        return self.rate_limited or self.needs_owner

    @property
    def run_outcome(self) -> str:
        # A rate-limited requeue is recorded as ``rate_limited`` so board history
        # doesn't show a phantom crash for a quota wall. An Owner-gated block gets its
        # own outcome for the same reason AND because ``check_respawn_guard`` has to
        # tell the two apart: one waits out a clock, the other waits for the Owner.
        if self.needs_owner:
            return "needs_owner"
...
def _classify_dead_worker(pid: int, claimer: Optional[str]) -> _DeadWorker:
    ...
    if kind == "needs_owner":
        # Owner-gated Connection — also NOT a task failure, and also not a quota wall. The error
        # text lands in ``tasks.last_failure_error`` and in the closed run, which is what
        # ``build_worker_context`` shows the next worker: it is the only warning that attempt N+1
        # is a re-run from step one over side effects attempt N may already have committed.
        return _DeadWorker(
            kind, code,
            f"pid {pid} stopped on a disconnected Connection that only the Owner can restore — "
            "requeued without counting a failure, and held until that connection reports healthy. "
            "This attempt starts over from the beginning: the previous attempt may already have "
            "run commands, sent requests or changed external state before it stopped. Verify what "
            "already happened before repeating any consequential step.",
            "needs_owner",
            {"pid": pid, "claimer": claimer, "exit_code": code},
            needs_owner=True,
        )
...
def _reclaim_dead_workers(conn: sqlite3.Connection) -> _CrashSweep:
    ...
    with _kb.write_txn(conn):
        ...
        for row in rows:
            ...
            if dead.released_without_failure or dead.protocol_violation:
                # Stamp last_failure_error WITHOUT touching ``consecutive_failures``:
                # a rate-limited or Owner-gated requeue must show ``check_respawn_guard``
                # a blocker; a below-budget protocol violation never reaches
            ...
            if dead.released_without_failure:
                (sweep.needs_owner if dead.needs_owner else sweep.rate_limited).append(row["id"])
            else:
...
def detect_crashed_workers(conn: sqlite3.Connection) -> list[str]:
    ...
    detect_crashed_workers._last_needs_owner = sweep.needs_owner  # type: ignore[attr-defined]
...
def _connection_fault_still_open(conn: sqlite3.Connection, task_id: str) -> bool:
    """Is the Connection this task stopped on still recorded broken?

    The worker stamped WHICH connection it was (``record_needs_owner_block``); this asks the one
    canonical health store — the same store Connections renders and the reconnect action clears —
    whether that row is still faulted.

    Two fail-closed fallbacks, both deliberate. Without a stamped ``connection_id`` (an older run, a
    worker that died before writing) the task is held while ANY connection is faulted: a task that
    has already proved it cannot finish is worth less than the side effects a doomed re-run would
    replay. And an unreadable store holds too — releasing on an error is how a 44-hour outage turns
    into 44 hours of re-dispatch. Both release the moment the store reads clean.

    ``open_faults``, not the raw store. Both fallbacks above hold work until the Owner acts, which
    is only defensible while the Owner *can* act. A fault keyed to a deleted profile is one nobody
    can clear — Connections cannot draw a row for an agent that is gone — so counting it here held
    every unstamped ``needs_owner`` task for good, with no cause the Owner could see and nothing to
    press. An orphan is not a connection the Owner has yet to fix; it is one that no longer exists.
    """
    try:
        connection_id = str(_kb.latest_needs_owner_block(conn, task_id).get("connection_id") or "")
    except Exception:
        connection_id = ""
    try:
        from agent.connection_health import open_faults
        faults = open_faults()
        if not connection_id:
            return bool(faults)
        # Scope-agnostic on purpose. The worker may have been serving a named profile (an MCP grant
        # is per-profile) while the Connections row the Owner fixes is the global one; the question
        # here is only "is this connection still broken", and any scope answering yes means yes.
        return any(
            isinstance(entry, dict) and entry.get("reason_code")
            and (entry.get("connection_id") == connection_id or key.split("/", 1)[-1] == connection_id)
            for key, entry in faults.items())
    except Exception:
        _kb._log.debug("respawn guard: connection health unreadable for %s", task_id, exc_info=True)
        return True
...
def check_respawn_guard(
    ...
    ``"connection_fault"`` (latest run ``needs_owner`` and the Connection it stopped on is STILL
    recorded broken — held until the fault clears, not until a timer expires), then
    ...
    # 0. Owner-gated Connection. Unlike a quota window this does not reopen on a clock: on the
    #    incident it stayed dead for 44 hours, and every re-dispatch inside that window burned a
    #    full turn AND re-ran the task from step one over side effects the previous attempt had
    #    already committed. So the breaker is keyed on the fault itself — the same canonical health
    #    store Connections and the reconnect action read — and releases the moment it clears
    #    (``record_healthy`` on the next good turn, or a verified reconnect).
    if latest_run is not None and latest_run["outcome"] == _kb.NEEDS_OWNER_EVENT_KIND:
        if _connection_fault_still_open(conn, task_id):
            return "connection_fault"
        # Fault cleared: return early so the stamped Owner-gated text cannot re-trap the task
        # through ``blocker_auth`` — same reason the rate-limit branch below returns early.
        return None
...
def _run_reclaim_phase(
    ...
    *,
    ...
    result.needs_owner.extend(getattr(detect_crashed_workers, "_last_needs_owner", []))
