...
def _cmd_work_get(args: argparse.Namespace) -> int:
    ...
    from agent.autonomy.owner_continuity import verification_candidates

    item = dict(item)
    item["verification_candidates"] = verification_candidates(item)
...
def _cmd_work_update(args: argparse.Namespace) -> int:
    ...
    if state in {"needs_owner", "failed", "cancelled"}:
        current = store.get_work(work_id)
        if current and current["refs"].get("owner_request"):
            from agent.autonomy.owner_continuity import request_finish

            if not reason and state != "needs_owner":
                raise ValueError(
                    "Owner outcome requires --waiting-reason with the visible result"
                )
            item = request_finish(work_id, reason or "Owner decision required", terminal=state)
            if item is None:
                return _print_json({"ok": False, "error": "Owner work changed before outcome; inspect current state"}, 2)
...
def _cmd_work_complete(args: argparse.Namespace) -> int:
    ...
    if current and current['refs'].get('owner_request'):
        from agent.autonomy.owner_continuity import request_finish
        item = request_finish(work_id,args.result)
    else:
        item = store.complete_work(work_id,args.result,material=not getattr(args,"quiet",False))
    if item is None:
        return _print_json({"ok": False, "error": "not found"}, 2)
    return _print_json(item)


def _cmd_work_wait(args):
    from agent.autonomy.owner_continuity import wait
    return _print_json(wait(args.id,until=args.until,event=args.event,child=args.child,
                           external=args.external,deadline=args.deadline))


def _cmd_work_verify(args):
    from agent.autonomy.owner_continuity import verify
    item = verify(args.id, file=args.file, tool_message_id=args.tool_message_id,
                  process_id=getattr(args, "process_id", None))
    if item is None:
        return _print_json({"ok": False, "error": "Owner work changed before verification; inspect current state"}, 2)
    return _print_json(item)
...
def _cmd_work_drop(args: argparse.Namespace) -> int:
    ...
    if current and current['refs'].get('owner_request'):
        from agent.autonomy.owner_continuity import request_finish
        item = request_finish(work_id, args.reason, terminal='cancelled')
    else:
...
def select_rollout_slugs(
    *,
    ...
    from agent.autonomy.missions import PILOT_PROFILES
    from agent.autonomy.paths import resolve_home
    from tools.bot_mode_probe import _hermes_root, _roster

    current = [name for name, _ in _roster(_hermes_root(resolve_home()))]
    if profile:
        name = str(profile).strip().lower()
        if name not in current:
            raise ValueError("rollout profile is missing, deleted or hidden")
        return [name]
    if all_profiles:
        return [slug for slug in current if slug != "default" and slug not in ISOLATED_ROLLOUT_SLUGS]
    return [slug for slug in PILOT_PROFILES if slug in current and slug not in ISOLATED_ROLLOUT_SLUGS]
...
def cmd_autonomy(args: argparse.Namespace) -> None:
    ...
    handlers = {
        ...
        "work-wait": _cmd_work_wait,
        "work-verify": _cmd_work_verify,
