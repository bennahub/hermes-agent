...
_ws_close_reason = late("_ws_close_reason", "hermes_cli.web_routers.chat_ws")
...
def observe_computer(request: Request, computer_id: str, body: LeaseBody):
    try:
        return get_contract().observe(
            ...
            wake_if_needed=body.wake_if_needed,
        )
    except AgentComputerError as exc:
        raise _http(exc) from exc
...
async def computer_stream(ws: WebSocket, computer_id: str):
    ...
    try:
        ...
        native_runtime = getattr(contract.service.runtime, "native_desktop", False) is True
        if live and (has_cdp or native_runtime):
            async def _screencast() -> None:
                try:
                    if native_runtime:
                        from gateway.agent_computer.stream import run_native_screencast
                        await run_native_screencast(live, handle, contract.service.runtime)
                    else:
