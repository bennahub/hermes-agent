...
def _apply_observed_fault_to_status(provider_id: str, status: Dict[str, Any]) -> Dict[str, Any]:
    """Overlay the runtime's recorded verdict onto an Accounts-tab status card.

    Every helper above answers from LOCAL METADATA, which structurally cannot see a revoked grant:
    the stored token keeps a valid shape, a future ``expiresAt`` and a refresh token long after the
    provider tore the grant down. So this card said "signed in" for all 44 hours, while
    ``/api/connections`` — which already carries this overlay — said the row needed re-auth. Two
    owner-facing surfaces disagreeing about one credential is exactly what the binding contract
    forbids, so they now read the one canonical store (``hermes_cli.connections.canonical_fault``).

    ``logged_in`` flips to False because that is the truth the provider gave us. The local fields
    (``source``, ``source_label``, ``token_preview``, ``expires_at``) are kept: a revoked credential
    still has to be findable and removable, and the disconnect path keys off ``source``.
    """
    if not isinstance(status, dict):
        return status
    try:
        from hermes_cli.connections import canonical_fault
        fault = canonical_fault(provider_id)
    except Exception:
        fault = None
    if not fault:
        return status
    reason = str(fault.get("reason_code") or "reauthorization_required")
    owner_action = str(fault.get("owner_action") or "reauthorize")
    overlay = {
        **status, "logged_in": False, "detail_code": reason, "owner_action": owner_action,
        "connection_state": "needs_configuration" if owner_action == "configure" else "needs_auth",
        # A bare retry cannot clear an Owner-gated fault; never offer one.
        "retryable": False, "observed_fault": True,
    }
    for key in ("first_seen_at", "last_seen_at"):
        if isinstance(fault.get(key), (int, float)):
            overlay[key] = float(fault[key])
    return overlay


def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    """Dispatch to the right status helper for an OAuth provider entry, then overlay the runtime's
    observed verdict so this surface and ``/api/connections`` cannot disagree."""
    return _apply_observed_fault_to_status(provider_id, _local_provider_status(provider_id, status_fn))


def _local_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    """Local-metadata status card for an OAuth provider entry (no health overlay)."""
    try:
