...
const state = {
  ...
  nativeDesktop:false, opening:false, retries:0, lastMoveAt:0, lastCursorAt:0, cursor:"default",
...
function applyLocation(loc){
  if(!loc) return;
  const native = state.nativeDesktop || loc.source === "browser_chrome";
  for(const id of ["urlToggle", "fsUrlToggle"]){
    $(id).classList.toggle("hidden", native);
    if(native) $(id).textContent = "Show URL";
  }
  if(native){
    for(const id of ["fullUrl", "fsFullUrl"]) $(id).classList.remove("open");
    $("origin").textContent = "Use Chrome’s address bar";
    $("fsOrigin").textContent = "Chrome address bar";
    $("fullUrl").textContent = ""; $("fsFullUrl").textContent = "";
    $("httpsMark").hidden = true;
    $("httpsMark").textContent = "BROWSER";
    $("httpsMark").classList.remove("off");
    $("urlBox").placeholder = "Open a destination…";
    return;
  }
  $("httpsMark").hidden = false;
...
function applyComputer(c){
  ...
  state.nativeDesktop = !!c.native_desktop;
...
async function openStream(){
  ...
  try{
    ...
    ws.onmessage = (ev) => {
      ...
      if(msg.type === "hello"){
        state.nativeDesktop = !!msg.native_desktop;
...
$("goUrl").onclick = () => {
  ...
  if(!url) return;
  if(state.nativeDesktop && !/^https?:\/\//i.test(url)){
    showErr("Enter a full http:// or https:// address, or use Chrome's address bar.");
    return;
  }
...
surface.addEventListener("pointerdown", (ev) => {
  if(!ownerHasControl()) return;
  if(!state.nativeDesktop && ev.button !== 0) return;
  ev.preventDefault();
  ...
  sendEvent(Object.assign({type:"pointer", phase:"down", buttons:ev.buttons, click_count: state.clickCount}, p));
  surface.focus();
});
surface.addEventListener("pointermove", (ev) => {
  ...
  sendEvent(Object.assign({type:"pointer", phase:"move", buttons: ev.buttons}, p));
});
surface.addEventListener("contextmenu", (ev) => { if(ownerHasControl() && state.nativeDesktop) ev.preventDefault(); });
function releaseNativeInputs(){
  state.drawing = false;
  if(ownerHasControl() && state.nativeDesktop) sendEvent({type:"release"});
}
surface.addEventListener("blur", releaseNativeInputs);
surface.addEventListener("pointercancel", releaseNativeInputs);
window.addEventListener("blur", releaseNativeInputs);
document.addEventListener("visibilitychange", () => { if(document.hidden) releaseNativeInputs(); });
...
surface.addEventListener("pointerup", (ev) => {
  ...
  const p = mapPoint(ev);
  if(!p) return;
  sendEvent(Object.assign({type:"pointer", phase:"up", buttons:ev.buttons, click_count: state.clickCount}, p));
});
...
surface.addEventListener("keydown", (ev) => {
  ...
  if((ev.metaKey || ev.ctrlKey) && (state.nativeDesktop ? "v" : "lrtwnv").includes(ev.key.toLowerCase())) return;
  ev.preventDefault();
