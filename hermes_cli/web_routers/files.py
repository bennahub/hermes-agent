...
_SENSITIVE_MANAGED_FILE_BASENAMES = path_authorization.SENSITIVE_FILE_BASENAMES
...
_SENSITIVE_MANAGED_DIR_NAMES = path_authorization.SENSITIVE_DIR_NAMES
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    """
    return path_authorization.is_sensitive_filename(name)
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    scope for this fix.
    """
    return path_authorization.is_sensitive_path(path)
...
def _managed_readable_file(request: Request, path: str) -> tuple[Any, Path, str, int, str]:
    ...
    _enforce_read_allowlist(policy, target)
...
async def download_published_artifact(
    request: Request, artifact_id: str, profile: str | None = None
):
    """Serve a file an agent explicitly published for the owner, by id.

    The difference from ``/api/files/download`` is the whole point of this
    route: it takes an **id the server minted**, not a path the caller chose.
    There is nothing to traverse, nothing to point at a credential store, and
    nothing for the client to know about the filesystem. Publication is the
    boundary — a file the runtime user merely *can* read is not reachable here
    until an agent has registered it.

    Deliberately **not** added to ``_QUERY_TOKEN_API_PATHS``: the mobile client
    sends its credential in a header, so widening the query-token exception
    would add a way to leak a session token into a URL for no gain.

    Unknown id, malformed id, and a record whose bytes have since gone all
    answer 404 identically, so the response cannot be used to probe which of
    the three it was.
    """
    ...
    found = published_artifacts.resolve_blob(profile_home, artifact_id)
    if found is None:
