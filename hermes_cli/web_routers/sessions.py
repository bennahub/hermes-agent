...
async def search_sessions(
    ...
    sources: str = None, exclude_sources: str = None, kind: str = "session", offset: int = 0):
    ...
    """
    result_kind = (kind or "session").strip().lower() or "session"
    if result_kind not in {"session", "file"}:
        raise HTTPException(status_code=400, detail="kind must be 'session' or 'file'")
    if not q or not q.strip():
        return {"results": [], **({"kind": "file", "has_more": False} if result_kind == "file" else {})}
    with http_failure("GET /api/sessions/search failed", 500, detail="Search failed"):
        ...
        try:
            ...
            if result_kind == "file":
                ...
                return search_profile_files(db, q, limit=safe_limit, offset=max(0, offset),
                                            include_sources=include_sources, exclude_sources=exclude_list)
            now = time.time()
...
def _project_for_display(messages: list) -> list:
    ...
    from agent.message_projection import compact_owner_collection, display_row, owner_safe_metadata

    projected_messages = []
    for message in messages:
        if not is_compaction_summary_message(message):
            projected_messages.append(display_row(message))
            continue
        ...
        if projected.get("display_metadata"):
            projected["display_metadata"] = owner_safe_metadata(projected["display_metadata"])
        projected_messages.append(projected)
    return compact_owner_collection(projected_messages)
...
async def rename_session_endpoint(session_id: str, body: SessionRename):
    ...
    if body.read_through is not None and body.unread is not False:
        raise HTTPException(status_code=400, detail="read_through requires unread=false")
    ...
    def _update(db):
        ...
        for flag, setter in _RENAME_FLAG_SETTERS:
            ...
            if value is not None:
                if flag == "unread" and body.read_through is not None:
                    db.set_session_read(sid, read=True, through=body.read_through)
                else:
                    setter(db, sid, value)
