...
def _read_query_file(args) -> None:
    ...
    if not (args.query or "").strip():
        ...
        sys.exit(2)
    # A native local DM's private sidecar carries display provenance only.
    # Ordinary query files and remote messages cannot choose their attribution.
    from agent.message_projection import stage_query_projection
    from tools.bot_mode_dm import read_dm_projection
    stage_query_projection(args.query, read_dm_projection(_qfile, args.query))
...
cmd_runtime = _forward_command("cmd_runtime", "hermes_cli.runtime_cmd", "runtime_command", doc='Show canonical hosted Hermes runtime status.')
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    _SUBCOMMANDS = {
        ...
        "auth", "status", "runtime", "cron", "doctor", "config", "pairing", "skills", "tools", "mcp",
...
_BUILTIN_SUBCOMMANDS = frozenset(
    {
        ...
        "resume",
        "send", "sessions", "setup",
        "skin", "skills", "slack", "status", "runtime", "sync", "tools", "uninstall", "update",
...
def _try_fast_serve_launch() -> bool:
    ...
    if (os.environ.get("HERMES_DISABLE_FAST_SERVE_LAUNCH") == "1"
            or "HERMES_EXECUTION_SCOPE" in os.environ):
        return False
...
def main():
    ...
    if hasattr(args, "func"):
        ...
        rc = dispatch_native_command(args)
        if isinstance(rc, int) and rc != 0:
            sys.exit(rc)
    else:
        parser.print_help()
