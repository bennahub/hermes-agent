...
_TICK_ACTIVITY_FIELDS = (
    ...
    "timed_out", "auto_blocked", "rate_limited", "needs_owner", "auto_assigned_default",
...
# Worker exit "blocked on an Owner-gated Connection": also released without counting a failure, but
# it is NOT a quota wall and must not share that code. A quota window reopens on its own in minutes,
# so ``rate_limit_cooldown`` re-dispatches on a timer and that is correct. An Owner-gated fault
# persists until the Owner re-consents in a browser — 44 hours, on the incident this taxonomy comes
# from — so the same timer would re-dispatch the task into a doomed turn every cooldown, forever,
# replaying whatever side effects the previous attempt had already committed. A distinct code lets
# the dispatcher hold the task against the fault itself (``check_respawn_guard``) instead of a clock.
# 76 == BSD EX_PROTOCOL, unused elsewhere in Hermes.
KANBAN_NEEDS_OWNER_EXIT_CODE = 76

#: ``task_events.kind`` for the worker's own record of an Owner-gated Connection block. Written by
#: the worker (which is the only process that knows WHICH connection broke and how far the turn got)
#: and read by ``check_respawn_guard``.
NEEDS_OWNER_EVENT_KIND = "needs_owner"


def record_needs_owner_block(
    conn: "sqlite3.Connection", task_id: str, *, connection_id: str = "", reason_code: str = "",
    owner_action: str = "", api_calls: int = 0, reason: str = "",
) -> None:
    """Record that this attempt stopped on an Owner-gated Connection, and how far it had got.

    Secret-free by construction: connection id, structural reason code, owner action, a call count
    and the owner-safe sentence. No credential, no provider prose.

    ``api_calls`` is the load-bearing field for the replay hazard. The incident turn hit its 401 at
    API call #17, after sixteen successful calls that had already run terminal commands, driven a
    browser and extracted from the web. A released task re-runs from the start, so the retry worker
    has to be told the previous attempt got that far — ``build_worker_context`` surfaces the run's
    error text to it.
    """
    _append_event(conn, task_id, NEEDS_OWNER_EVENT_KIND, {
        "connection_id": str(connection_id or ""), "reason_code": str(reason_code or ""),
        "owner_action": str(owner_action or ""), "api_calls": max(0, int(api_calls or 0)),
        "reason": str(reason or "")[:500],
    }, run_id=_current_run_id(conn, task_id))


def latest_needs_owner_block(conn: "sqlite3.Connection", task_id: str) -> dict:
    """Payload of the newest :data:`NEEDS_OWNER_EVENT_KIND` event for *task_id* (``{}`` if none)."""
    return _json_dict(_row_get(_latest_event(conn, task_id, NEEDS_OWNER_EVENT_KIND), "payload"))
...
def _resume_status_from_events(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    row = conn.execute(
        ...
        "'stale', 'timed_out', 'crashed', 'spawn_failed', 'rate_limited', 'needs_owner'"
        ") ORDER BY id DESC LIMIT 1", (task_id,),
    ).fetchone()
