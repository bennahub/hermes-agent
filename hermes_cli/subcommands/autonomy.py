...
def build_autonomy_parser(subparsers) -> None:
    ...
    update.add_argument("--waiting-reason", dest="waiting_reason", help="Visible reason/result; required for Owner failed or cancelled outcomes")
    update.add_argument("--objective")
    update.add_argument("--jira", help="Bind or record the Jira tracking identity")

    wait = sub.add_parser("work-wait", help="Register durable Owner task resume before promising to return")
    wait.add_argument("id")
    condition = wait.add_mutually_exclusive_group(required=True)
    condition.add_argument("--until")
    condition.add_argument("--event")
    condition.add_argument("--child")
    condition.add_argument("--external")
    wait.add_argument("--deadline")

    verify = sub.add_parser("work-verify", help="Record actual file or native tool-observation evidence")
    verify.add_argument("id")
    evidence = verify.add_mutually_exclusive_group(required=True)
    evidence.add_argument("--file")
    evidence.add_argument("--tool-message-id")
    evidence.add_argument("--process-id", help="Verify a successful native completion of this work's exact background process")
