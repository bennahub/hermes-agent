...
class SessionRename(BaseModel):
    ...
    read_through: Optional[float] = None  # optional observed reply cursor, only with unread=False
    profile: Optional[str] = None  # session owned by another profile (opens its state.db)

    @field_validator("read_through", mode="before")
    @classmethod
    def validate_read_through(cls, value):
        if value is not None and (isinstance(value, bool) or not isinstance(value, (int, float)) or not math.isfinite(value) or value < 0):
            raise ValueError("read_through must be finite non-negative seconds")
        return value
