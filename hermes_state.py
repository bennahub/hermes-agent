...
from hermes_state_execution_scopes import SessionExecutionScopesMixin
...
def divert_session_transcript_jsonl(session_id: str, messages) -> "Optional[Path]":
    ...
    ``mkdir_under_hermes_home``, never a bare ``mkdir(parents=True)``: "state.db was replaced
    under a live process" is exactly what a profile DELETE looks like from inside a turn, and
    the bare mkdir wrote a fragment of the permanently-deleted conversation back under the
    deleted profile, recreating its tree after ``rmtree`` walked past. Raising here is the safe
    outcome — both callers already catch and log the divert failure."""
    sid = str(session_id or "").strip()
    if not sid or not messages:
        return None
    sessions_dir = mkdir_under_hermes_home(get_hermes_home() / "sessions")
...
class SessionDB(
    ...
    SessionMessagesMixin, SessionExecutionScopesMixin,
):
