...
class SessionSearchMixin:
    ...
    _MESSAGE_TEXT_MAX_IDS = 1000

    def get_message_texts(
        self,
        message_ids: Collection[int],
        *,
        roles: Optional[Collection[str]] = None,
    ) -> Dict[int, str]:
        """Return ``{message_id: plain text}`` for a bounded set of ids.

        No search SELECT projects ``content`` — multi-MB tool rows would
        dominate the I/O of every query, which is why
        :meth:`_finalize_search_matches` pops the column outright. Callers that
        need the text of a handful of *already matched* rows read it here
        instead: a primary-key lookup over an explicit, capped id list,
        optionally narrowed to the small roles (``user``) that carry the data.

        Ids that do not exist, or that fail the ``roles`` filter, are simply
        absent from the result.
        """
        ids = [int(mid) for mid in message_ids][: self._MESSAGE_TEXT_MAX_IDS]
        if not ids:
            return {}
        where = [f"id IN ({','.join('?' for _ in ids)})"]
        params: List[Any] = list(ids)
        role_list = [str(role) for role in (roles or ())]
        if role_list:
            where.append(f"role IN ({','.join('?' for _ in role_list)})")
            params.extend(role_list)
        sql = f"SELECT id, content FROM messages WHERE {' AND '.join(where)}"
        with self._read_ctx() as conn:
            rows = conn.execute(sql, params).fetchall()
        return {row["id"]: self._plain_text_content(row["content"]) for row in rows}


    @classmethod
    def _plain_text_content(cls, content: Any) -> str:
        """Flatten a stored ``content`` value to plain text.

        Native-vision turns persist ``content`` as a JSON parts list; only the
        text parts carry the ``@image:``/``@file:`` directives.
        """
