...
@contextlib.contextmanager
def _jobs_lock(*, required: bool = False):
    ...
    if depth:
        if required and not getattr(_jobs_lock_state, "cross_process_acquired", False):
            raise RuntimeError("Cron admission requires the cross-process jobs lock")
    ...
    with _jobs_file_lock:
        ...
        try:
            try:
                ...
                _jobs_lock_state.cross_process_acquired = acquired is True
            ...
            try:
                if required and not _jobs_lock_state.cross_process_acquired:
                    raise RuntimeError("Cron admission requires the cross-process jobs lock")
                yield
            finally:
...
_IMMUTABLE_JOB_FIELDS = frozenset({"id", "execution_scope", "native_continuation"})
...
def create_job(
    ...
    native_continuation: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    ...
    if native_continuation is not None:
        job["native_continuation"] = dict(native_continuation)
...
def update_job(job_id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def apply(jobs, i, job):
        ...
        _reject_terminal_activation(job, updated, job_id)
        from cron.execution_scope import refresh_job_scope, close_previous_job_scope

        previous_scope = refresh_job_scope(job, updated)
        jobs[i] = updated
        save_jobs(jobs)
        close_previous_job_scope(previous_scope)
...
def heartbeat_fire_claim(job_id: str, *, expected_owner: str) -> bool:
    # Heartbeat renewal must not contend with its own external-I/O fire fence.
    def apply(jobs, _i, job):
