...
def get_execution(
    execution_id: str, *, store_home: Optional[Path] = None,
) -> Optional[Dict[str, Any]]:
    """Return one exact attempt; explicit native homes never mutate global routing."""
    if store_home is not None:
        path = Path(store_home).resolve() / "cron" / "executions.db"
        if not path.is_file():
            return None
