...
def run_job(
    ...
    # One identity across monitor, pre-run script and agent work for this fire.
    job = {**job, "execution_id": execution_id or job.get("execution_id") or uuid.uuid4().hex}
    ...
    try:
        ...
        if _session_db is not None:
            _session_db.create_session(_cron_session_id, source="cron", hidden=True)
...
def _save_compose_deliver(
    ...
    continuity_response = (job_has_continuity(job) and d.success and final_response.strip()
    ...
    def persist_continuity():
        if continuity_response and not d.delivery_error and not _is_interrupted(job["id"], execution_token):
            with fence.side_effect_fence() as owns_continuity:
...
def tick(
    ...
    try:
        ...
        # Explicit Owner continuation is independent of optional observation jobs.
        # This runs after the same native drain/ESTOP gates, under the tick lock.
        try:
            from agent.autonomy.owner_continuity import reconcile as reconcile_owner_work
            reconcile_owner_work()
        except Exception:
            logger.exception("Owner responsibility reconciliation deferred; continuing native cron tick")
