...
def _run_monitor_source(job: dict) -> tuple[bool, str]:
    ...
    if monitor_script:
        ...
        return run_job_script_scoped(job, monitor_script, workdir=_field(job, "workdir") or None, step="monitor")
    monitor_url = _field(job, "monitor_url")
    if monitor_url:
        from cron.execution_scope import execute_job_action

        return execute_job_action(job, "monitor", "web_fetch", {"url": monitor_url},
                                  lambda args: _fetch_monitor_url(args["url"]))
