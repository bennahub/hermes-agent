        ...
        def _do(conn):
            ...
            conn.execute(
                """INSERT INTO sessions (
                   ...
                   origin_json, display_name, started_at, hidden
                )
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
""" + _UPSERT_KEEP_EXISTING_SQL,
    ...
    def _set_lineage_column(self, column: str, session_id: str, value: Any, *, monotonic: bool = False) -> bool:
        ...
        assigned = (f"MAX(COALESCE((SELECT MAX({column}) FROM sessions WHERE id IN (SELECT id FROM lineage)), 0), ?)"
                    if monotonic else "?")
        return self._write_rowcount(
            f"""
            WITH RECURSIVE
    ...
    def set_session_read(self, session_id: str, read: bool = True, *, through: Optional[float] = None) -> bool:
        """Mark read/unread in the existing compression lineage.

        Legacy callers retain read-now / explicit-unread behavior. A client with
        an observed transcript cursor may read only through that timestamp; a
        delayed request must not consume a newer unseen reply or regress a read
        already recorded by another client. Explicit unread still resets to 0.
        """
        if through is not None:
            if not read or isinstance(through, bool) or not isinstance(through, (int, float)) or not math.isfinite(through) or through < 0:
                raise ValueError("read cursor must be finite non-negative seconds and requires read=True")
            return self._set_lineage_column("last_read_at", session_id, float(through), monotonic=True)
