...
def finalize_turn(
    ...
    def _persist_step():
        ...
        if final_response and not interrupted:
            ...
            # A display lookup cannot turn an executed native turn into a failure.
            with suppress(Exception):
                stamp_final(agent, messages)
        if not interrupted and not failed:
            _micro_compact_after_turn(agent, messages, final_response, logger)
        with suppress(Exception):
            _attach_published_artifacts(agent, messages)
        if not interrupted and not failed:
            from agent.message_projection import record_peer_final
            # The recipient result and its finalized artifact cards must be in
            # the collaboration ledger before a live callback or background
            # process completion can wake the parent.
            with suppress(Exception):
                record_peer_final(agent, messages)
...
def _attach_published_artifacts(agent, messages) -> None:
    """Stamp this turn's published files onto its final assistant message.

    Stamped in **two** places, because they cover different halves of the same
    turn. The in-memory dict is what an in-flight client render and any
    not-yet-flushed row will carry. The durable row is what everything else
    reads — and the session flush is append-only, so a row already written
    during the turn never learns about a dict mutated after it. Only the first
    of those existed at first, and the owner's phone showed the result: an agent
    saying "the file is attached" above a message with no attachment on it.

    Silent when the turn published nothing, which is almost every turn.
    """
    from gateway import published_artifacts

    session_id = getattr(agent, "session_id", "")
    references = published_artifacts.take_pending(session_id)
    if not references:
        return
    ...
    for message in reversed(messages):
        ...
        metadata["attachments"] = (
            list(existing) + references if isinstance(existing, list) else references
        )
        ...
        if db is not None and session_id:
            try:
                # Idempotent by artifact id, so a row that did learn about the
                # dict through a later flush does not end up with two cards for
                # one file.
                db.attach_artifacts_to_message(
                    session_id,
                    role="assistant",
                ...
                logger.debug("failed to persist artifact attachment", exc_info=True)
        return
