...
def _record_refresh_fault(reason_code: str = "refresh_failed") -> None:
    """Publish a rejected refresh to the canonical Connection health store (fail-soft, secret-free).

    Refresh failure is the moment Hermes *knows* the grant is dead. Recording it here — rather than
    waiting for a model call to 401 — is what lets Connections and Needs You tell the truth even
    when no agent has run since.
    """
    try:
        from agent.connection_health import connection_block_dict, record_fault
        record_fault(connection_block_dict(
            provider="anthropic", base_url="https://api.anthropic.com", reason_code=reason_code,
        ))
    except Exception:
        pass
    ...
    try:
        ...
        with _auth_store_lock(timeout_seconds=lock_timeout_seconds, target_path=cred_path):
            ...
            try:
                ...
                # WARNING, not debug: a rejected refresh means the Owner's grant is gone and every
                # subsequent model call will fail. At debug level this was invisible, and the
                # connection stayed broken for 44 hours with no operator signal at all.
                # ``type(e).__name__`` only — an exception's str() can carry the token it POSTed.
                logger.warning(
                    "Claude Code OAuth refresh was rejected (%s) — the Anthropic connection needs "
                    "the Owner to reauthorize; recording it as a Connection fault",
                    type(e).__name__,
                )
                _record_refresh_fault()
                return None
