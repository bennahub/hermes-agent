...
def _enrich_contextual_followup(agent: Any, user_message: Any, conversation_history: Any) -> Any:
    """Bind كمل / أقصد هذي / reply-like follow-ups to the live referent for every surface."""
    if not isinstance(user_message, str) or user_message.startswith("[Continuing the referenced"):
        return user_message
    try:
        from agent.conversation_referents import (
            AmbiguousReferent, followup_quote, is_contextual_followup, resolve_conversation_referent,
        )
        if not is_contextual_followup(user_message):
            return user_message
        rows = []
        ...
        if db is not None and session_id:
            try:
                rows = list(db.get_messages(session_id) or [])
            except Exception:
                rows = []
        if not rows and conversation_history:
            rows = [row for row in conversation_history if isinstance(row, dict)]
        quote = followup_quote(resolve_conversation_referent(rows, user_message))
    except AmbiguousReferent:
        return user_message
    except Exception:
        return user_message
    return (quote + user_message) if quote else user_message
...
def _stage_turn_user_message(
    ...
    from agent.message_projection import native_metadata, projection, consume_query_projection
    query_metadata = consume_query_projection(expected_persist_content, agent)
    if query_metadata is not None:
        user_msg["display_metadata"] = query_metadata
        user_msg["display_kind"] = "a2a_message"
    from agent.autonomy.owner_continuity import continuation_display_metadata
    continuation_metadata = continuation_display_metadata(agent)
    if continuation_metadata is not None:
        user_msg["display_kind"] = "hidden"
        user_msg["display_metadata"] = native_metadata(
            "runtime", "internal", "control", metadata=user_msg.get("display_metadata"),
            **continuation_metadata)
    agent._turn_display_projection = projection(user_msg.get("display_metadata"))
...
def build_turn_context(
    ...
    compression."""
    from agent.interactive_timing import stage_emitter
    _record_stage = stage_emitter(agent)
    _record_stage("CONTEXT_BUILD_START")
    ...
    _discard_stale_published_artifacts(agent)
    try:
        from gateway.model_routing import order_fallback_chain
        chain = getattr(agent, "_fallback_chain", None)
        if chain:
            agent._fallback_chain = order_fallback_chain(
                chain, primary_provider=str(getattr(agent, "provider", "") or ""))
    except Exception:
        pass
    ...
    user_message = _enrich_contextual_followup(agent, user_message, conversation_history)
    ...
    _bind_owner_task_authority(agent, turn_id, persist_user_display_kind)
    ...
    from agent.message_projection import projection as message_projection
    peer_metadata = messages[current_turn_user_idx].get("display_metadata") or {}
    peer_projection = message_projection(peer_metadata)
    if peer_projection and peer_projection["origin"] == "peer" and peer_metadata.get("attachments"):
        from gateway.a2a_artifacts import recipient_context
        plugin_user_context += recipient_context(agent, peer_metadata)
    ...
    prepare_scope(agent, original_user_message, messages, current_turn_user_idx, persist_user_display_kind)
    from agent.autonomy.owner_continuity import bind_turn as bind_owner_continuity
    owner_continuity_hint = bind_owner_continuity(
        agent, messages[current_turn_user_idx], display_kind=persist_user_display_kind)
    if owner_continuity_hint:
        plugin_user_context = (plugin_user_context + owner_continuity_hint).strip()
        _stamp_api_content_sidecar(agent, messages, current_turn_user_idx,
            ext_prefetch_cache, plugin_user_context, preflight_compressed=False)
        if getattr(agent, "_session_db", None) is not None:
            agent._session_db.set_latest_user_api_content(agent.session_id,
                messages[current_turn_user_idx].get("content"),
                messages[current_turn_user_idx].get("api_content"))

    from agent.execution_scope import bind_agent_turn
    bind_agent_turn(agent, original_user_message, messages[current_turn_user_idx],
    ...
    _record_stage("CONTEXT_BUILD_DONE")
    return TurnContext(
...
def _discard_stale_published_artifacts(agent) -> None:
    """Drop anything a previous, unfinished turn published.

    `finalize_turn` is the only drain, and `conversation_loop` documents four
    paths that never reach it. Left alone, a `send_file` on one of those hands
    its file to the *next* turn in that session — the owner asks about the
    weather and the reply carries a .docx card. Discarded at the start of a turn
    rather than chased through every exit: a turn can only ever deliver its own
    files, and this is the one place every turn passes through.

    The artifact itself is untouched and still downloadable by id; only the
    claim on a message is dropped.
    """
    try:
        from gateway import published_artifacts

        stale = published_artifacts.take_pending(getattr(agent, "session_id", ""))
        if stale:
            logger.info(
                "Discarded %d published artifact(s) from a turn that never "
                "finalized; they remain downloadable by id.",
                len(stale),
            )
    except Exception:
        pass
...
def _bind_owner_task_authority(agent, turn_id: str, persist_user_display_kind=None) -> None:
    """Consume only the nonce dispatched for this owner turn after identity binding."""
    _owner_task_nonce = getattr(agent, "_pending_owner_task_nonce", None)
    agent._pending_owner_task_nonce = None
    if (not getattr(agent, "_persist_disabled", False)
            and not str(persist_user_display_kind or "").strip() and _owner_task_nonce):
        try:
            from tools import owner_task_authority
            from tools.approval_context import get_current_session_key
            claimed = owner_task_authority.bind_turn(
                [getattr(agent, "session_id", None), get_current_session_key()], turn_id,
                nonce=_owner_task_nonce)
            if not claimed and not getattr(agent, "_pending_execution_source", None):
                agent._pending_execution_source = _owner_task_nonce
        except Exception:
            if not getattr(agent, "_pending_execution_source", None):
                agent._pending_execution_source = _owner_task_nonce
