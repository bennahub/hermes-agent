...
def preprocess_context_references(
    ...
    allowed_file_paths: tuple[str | Path, ...] = (),
) -> ContextReferenceResult:
    """Sync wrapper; safe both without a loop (CLI) and inside a running loop (gateway)."""
    coro = preprocess_context_references_async(
        message, cwd=cwd, context_length=context_length, url_fetcher=url_fetcher, allowed_root=allowed_root, allowed_file_paths=allowed_file_paths
    )
    try:
...
async def preprocess_context_references_async(
    ...
    allowed_files = frozenset(Path(path).absolute() for path in allowed_file_paths)
    ...
    tasks = (
        _expand_reference(ref, cwd_path, url_fetcher=url_fetcher, allowed_root=allowed_root_path, allowed_file_paths=allowed_files,
...
async def _expand_reference(
    ...
    allowed_file_paths: frozenset[Path] = frozenset(),
) -> Expansion:
    try:
        if ref.kind in ("file", "folder"):
            return _expand_path_reference(ref, cwd, allowed_root=allowed_root, max_inline_tokens=max_inline_tokens,
                                          allowed_file_paths=allowed_file_paths if ref.kind == "file" else frozenset())
...
def _expand_path_reference(ref: ContextReference, cwd: Path, *, allowed_root: Path | None = None,
                           max_inline_tokens: int | None = None,
                           allowed_file_paths: frozenset[Path] = frozenset()) -> Expansion:
    """``@file:`` / ``@folder:``: resolve, allow-check, then inline text / binary stub / listing."""
    is_folder = ref.kind == "folder"
    path = _resolve_path(cwd, ref.target, allowed_root=allowed_root, allowed_file_paths=allowed_file_paths)
...
def _resolve_path(cwd: Path, target: str, *, allowed_root: Path | None = None,
                  allowed_file_paths: frozenset[Path] = frozenset()) -> Path:
    resolved = (cwd / Path(os.path.expanduser(target))).resolve()  # `/` keeps an absolute target as-is
    if allowed_root is not None and resolved not in allowed_file_paths and not _is_under(resolved, allowed_root):
