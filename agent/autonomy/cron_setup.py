...
runtime_root = Path(sys.executable).absolute().parents[2]
if (runtime_root / "agent").is_dir():
...
def _observe_job_fields(slug: str, expr: str) -> Dict[str, Any]:
    # Native cron resolves its deliberate profile defaults and fallback chain
    # at fire time. Installation must not invent a provider-only fallback or
    # overwrite an existing job's explicit provider/model pair.
    return {
