...
TERMINAL_STATES = ("completed", "dropped", "failed", "cancelled")
...
def start_work(
    *,
    ...
    if owner_assigned:
        from agent.autonomy.owner_continuity import validate_transition
        validate_transition(None, {"state": chosen, "refs": refs})
    if not owner_assigned and open_count >= MAX_OPEN_WORK and chosen in OPEN_STATES:
        chosen = "dropped"
    ...
    try:
        with _transaction(hermes_home) as conn:
            ...
            if owner_assigned:
                stopped = _owner_source_stop(conn, (refs or {}).get("owner_request"))
                if stopped:
                    source_refs = {**(refs or {}), "owner_stop": stopped,
                                   "pending_owner_result": _owner_stop_result(stopped)}
                    item["refs_json"] = json.dumps(source_refs, ensure_ascii=False)
                    item["completion_result"] = source_refs["pending_owner_result"]["text"]
                    item["state"] = "working"
            ...
            if not owner_assigned and open_count >= MAX_OPEN_WORK and (state or "working") in OPEN_STATES:
                if enforce_admission:
                    return {"created": False, "blocked": True, "reason": "open_work_cap"}
                item["state"] = "dropped"
            if enforce_admission and not owner_assigned:
                if parent_id:
            ...
            initial = {"state": item["state"], "refs": json.loads(item["refs_json"])}
            item["refs_json"] = json.dumps(preserve(None, initial), ensure_ascii=False)
            conn.execute(
                """INSERT INTO work (
...
def update_work(
    ...
    *,
    ...
    expected_refs: Optional[Dict[str, Any]] = None,
    **_ignored: Any,
    ...
    with _transaction(hermes_home) as conn:
        ...
        if row is None:
            return None
        current = _row_to_work(row)
        if expected_state is not None and current["state"] != expected_state:
            return None
        if expected_refs and any(current["refs"].get(k) != v for k, v in expected_refs.items()):
            return None
        if state is not None and state not in WORK_STATES:
            raise ValueError(f"invalid work state: {state}")
        if current["state"] in TERMINAL_STATES:
            if current["refs"].get("owner_request") or (state is not None and state not in TERMINAL_STATES):
                return current
        merged = {**current["refs"], **(refs or {})}
        desired = {**current, "state": state or current["state"], "refs": merged,
                   "completion_result": completion_result if completion_result is not None else current["completion_result"]}
        from agent.autonomy.owner_continuity import validate_transition
        from agent.autonomy.lifecycle import preserve
        merged = preserve(current, desired)
        validate_transition(current, desired)
        fields = {}
        if state is not None: fields["state"] = state
        if waiting_reason is not None: fields["waiting_reason"] = waiting_reason
        if objective is not None: fields["objective"] = _clip(objective, MAX_OBJECTIVE_CHARS)
        if completion_result is not None: fields["completion_result"] = _clip(completion_result, MAX_TEXT_CHARS)
        fields["refs_json"] = json.dumps(merged, ensure_ascii=False)
        if not fields: return current
        fields["updated_at"] = _now()
        conn.execute(f"UPDATE work SET {', '.join(f'{key}=?' for key in fields)} WHERE id=?",
                     [*fields.values(), work_id])
        return _row_to_work(conn.execute("SELECT * FROM work WHERE id=?", (work_id,)).fetchone())
...
def _owner_source_stop(conn, source):
    """Read under the same transaction as admission; later Owner rows remain valid."""
    if not isinstance(source, dict):
        return None
    row = conn.execute("SELECT value FROM meta WHERE key=?", ("owner-stop:" + source["session_id"],)).fetchone()
    if row is None:
        return None
    stopped = json.loads(row["value"])
    return stopped if int(source["message_id"]) <= stopped["through_message_id"] else None


def _owner_stop_result(stopped):
    return {"id": uuid.uuid4().hex, "state": "cancelled", "text": "Recorded your stop request. Further automatic continuation of this task is disabled.",
            "created_at": stopped["at"], "after_message_id": stopped["through_message_id"]}


def record_owner_stop(cutoffs, hermes_home=None):
    """Native Stop fence and existing work cancellation share one ledger transaction.

    Cutoffs are read from native canonical lineage rows by the authenticated
    ingress. Admission serializes here too, including late automatic promotion.
    No permanent session blacklist: a new source row above the cutoff is valid.
    """
    import time
    affected = []
    with _transaction(hermes_home) as conn:
        for session_id, message_id in cutoffs.items():
            key = "owner-stop:" + session_id
            existing = conn.execute("SELECT value FROM meta WHERE key=?", (key,)).fetchone()
            prior = json.loads(existing["value"]) if existing else None
            if prior and prior["through_message_id"] >= message_id:
                continue
            stopped = {"session_id": session_id, "through_message_id": int(message_id), "at": time.time()}
            conn.execute("INSERT INTO meta(key,value,updated_at) VALUES(?,?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=excluded.updated_at",
                         (key, json.dumps(stopped), _now()))
        rows = conn.execute("SELECT * FROM work").fetchall()
        for row in rows:
            work = _row_to_work(row)
            source = work["refs"].get("owner_request") or {}
            if source.get("session_id") not in cutoffs or work["state"] not in OPEN_STATES:
                continue
            stopped = _owner_source_stop(conn, source)
            if not stopped:
                continue
            if not work["refs"].get("owner_stop"):
                pending = _owner_stop_result(stopped)
                refs = {**work["refs"], "owner_stop": stopped, "pending_owner_result": pending}
                from agent.autonomy.lifecycle import preserve
                refs = preserve(work, {"state": work["state"], "refs": refs})
                conn.execute("UPDATE work SET refs_json=?,completion_result=?,updated_at=? WHERE id=?",
                             (json.dumps(refs, ensure_ascii=False), pending["text"], _now(), work["id"]))
