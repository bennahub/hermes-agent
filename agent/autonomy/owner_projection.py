...
def _insert_message(
    ...
    if _has(cols, "display_kind"):
        fields["display_kind"] = "hidden" if role == "user" else _NOTICE_KIND
    if _has(cols, "display_metadata"):
        from agent.message_projection import native_metadata
        fields["display_metadata"] = json.dumps(native_metadata(
            "runtime" if role == "user" else "agent",
            "internal" if role == "user" else "owner",
            "control" if role == "user" else "decision", source_session_id=session_id))
