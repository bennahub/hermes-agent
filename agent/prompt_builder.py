...
PLATFORM_HINTS = {
    ...
    "tui": (
        # MEDIA: still is not intercepted in tui/ (maintainer-confirmed) —
        # tags print literally, so that half of the old wording stands.
        #
        # What changed: gateway sessions are stamped platform="tui", so the
        # mobile client reads this hint too, and it used to end at "state the
        # path". That is now wrong there and was actively harmful: asked for a
        # document, the agent pasted `/home/hermes/…` and said it could not
        # attach it, while `send_file` sat unused in its own tool list. The
        # tool's presence is the honest signal — a surface that has it has a
        # real attachment channel, and one that does not still needs the path.
        "You are in the Hermes terminal UI (TUI). Files: MEDIA:/path tags "
        "are NOT intercepted here (they print as literal text). If you have "
        "the send_file tool, that is how you deliver a file — the owner "
        "receives it as a real attachment on your reply, so call it and "
        "describe the file in words; do NOT paste the path instead, and do "
        "not offer email or a download link as a substitute. Without that "
        "tool there is no attachment channel: deliver a file by stating its "
        "absolute path or URL in plain text. "
        + _LOCAL_CRON_DELIVERY_NOTE
    ),
    "desktop": (
