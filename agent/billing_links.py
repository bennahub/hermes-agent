...
def provider_display_label(provider: str, base_url: str = "") -> str:
    """Human-readable provider name for any owner-facing surface (slug → base_url host → title-cased slug).

    Shared with :mod:`agent.connection_health` so a billing wall and a connection fault name the same
    provider identically on every client.
    """
    slug = (provider or "").strip().lower()
    if is_nous_inference_route(slug, base_url):
        return "Nous Portal"
    return _resolve_provider_link(slug, base_url)[0]
