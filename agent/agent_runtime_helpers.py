...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    if inline_executor is not None:
        ...
        def _execute(next_args: dict) -> Any:
            from agent.execution_scope import execute_scoped
            result = execute_scoped(function_name, next_args,
                lambda admitted: inline_executor(agent, admitted, inline_ctx),
                turn_id=getattr(agent, "_current_turn_id", None),
                invocation_id=tool_call_id, registry=True)
            emit_terminal_post_tool_call(
...
def clear_turn_authority(agent):
    """Drop any owner-task or task-scoped authority bound to this turn.

    Called at every turn-EXIT point — the normal `finalize_turn` and the early
    returns that bypass it (preflight-compression timeout, the codex app-server
    handoff) — so a grant never outlives the turn it was minted for. Deliberately
    NOT called from the mid-turn persist funnel (`note_turn_persisted`), which
    runs many times inside one turn: a grant must survive a save. Idempotent, so
    calling it more than once is harmless.
    """
    turn_id = getattr(agent, "_current_turn_id", "") or None
    if not turn_id:
        return
    from agent.execution_scope import close_turn
    close_turn(agent)
    try:
        from tools import owner_task_authority as _owner_authority
        _owner_authority.clear_turn(turn_id)
    except Exception:
        pass
    try:
        from tools import file_tools_write_guards as _file_tools
        # The explicit "Approve for this task" grants, keyed by turn — cleared
        # here rather than left to the six-hour cap, which was the only thing
        # bounding them since no production code called this.
        _file_tools.clear_task_instruction_authority(turn_key=turn_id)
    except Exception:
        pass
