...
def nonretryable_client_error_result(
    ...
    from agent.conversation_loop import (
        _CONTENT_POLICY_RECOVERY_HINT, _billing_failure_result, _connection_failure_result,
    ...
    # A terminal auth failure is a broken Connection, not an answer. Without this branch the
    # provider's own words ("HTTP 401: OAuth access token has been revoked.") became the
    # ``final_response`` and were delivered to the Owner as an ordinary agent reply.
    if classified.is_auth:
        # ``agent``/``conversation_history``: the verdict must also land in the durable transcript,
        # or the failed turn leaves no history row at all and a relaunch shows the Owner their own
        # prompt with no answer and no reconnect affordance.
        return _connection_failure_result(
            classified=classified, summary=_nonretryable_summary, messages=messages,
            api_call_count=api_call_count, provider=provider, base_url=base_url, model=model,
            status_code=status_code, agent=agent, conversation_history=conversation_history,
        )
...
def max_retries_exhausted_result(
    ...
    agent._persist_session(messages, conversation_history)
    # An auth failure that survived every retry, rotation and fallback is a broken Connection.
    # Return the same structured verdict as the non-retryable path so both terminal routes give the
    # Owner one signal, and neither leaks the provider's raw 401 prose as the answer.
    if classified.is_auth:
        from agent.conversation_loop import _connection_failure_result
        return _connection_failure_result(
            classified=classified, summary=_final_summary, messages=messages,
            api_call_count=api_call_count, provider=provider, base_url=base_url, model=model,
            status_code=getattr(api_error, "status_code", None) or classified.status_code,
            agent=agent, conversation_history=conversation_history,
        )
    _billing_block = None
