...
def _canonical_attachment_text_override(msg: Dict, content: Any, override: Any) -> bool:
    """True only for a native Owner attachment turn whose clean transcript is text.

    The gateway sends image-capable providers a content-part list, while the native
    attachment ledger remains the durable source of the files shown to the Owner.
    Requiring the validated native projection, batch identity, client identity and every
    artifact identity keeps an arbitrary list-valued message from losing media.
    """
    if not isinstance(content, list) or not isinstance(override, str):
        return False
    from agent.message_projection import metadata_dict, projection
    metadata = metadata_dict(msg.get("display_metadata"))
    native = projection(metadata)
    attachments = metadata.get("attachments")
    client_ids = metadata.get("client_message_ids")
    return bool(
        native
        and native.get("origin") == "owner"
        and native.get("audience") == "owner"
        and native.get("purpose") == "message"
        and isinstance(metadata.get("attachment_batch_id"), str)
        and metadata["attachment_batch_id"]
        and isinstance(client_ids, list)
        and len(client_ids) == 1
        and isinstance(client_ids[0], str)
        and client_ids[0] == metadata["attachment_batch_id"]
        and isinstance(attachments, list)
        and attachments
        and all(isinstance(item, dict) and isinstance(item.get("artifact_id"), str)
                and item["artifact_id"] for item in attachments)
    )


def _override_replaces_content(msg: Dict, content: Any, override: Any) -> bool:
    """May the persist override replace ``content`` without dropping durable media?

    A list override is itself the clean multimodal payload. A text override is also
    valid for a canonical native attachment batch because its files live in the
    attachment ledger and the exact text-form replay is retained in ``api_content``.
    Compaction summary carriers are never overwritten.
    """
    return (
        ...
        and (
            ...
            or _canonical_attachment_text_override(msg, content, override)
        )
    )
    ...
    if is_current_turn_user and role == "user":
        ...
        if _override_replaces_content(msg, content, override):
            ...
            if api_content is None and isinstance(content, str) and content != override:
                api_content = content
            elif api_content is None and _canonical_attachment_text_override(msg, content, override):
                durable_wire = _durable_content(content)
                if isinstance(durable_wire, str) and durable_wire != override:
                    api_content = durable_wire
            content = override
...
class SessionPersistenceMixin:
    ...
    def _apply_persist_user_message_override(self, messages: List[Dict]) -> None:
        ...
        if _override_replaces_content(msg, original_content, override):
            if _canonical_attachment_text_override(msg, original_content, override):
                durable_wire = _durable_content(original_content)
                if (
                    not isinstance(msg.get("api_content"), str)
                    and isinstance(durable_wire, str)
                    and durable_wire != override
                ):
                    msg["api_content"] = durable_wire
