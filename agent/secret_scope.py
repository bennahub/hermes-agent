...
def get_secret(name: str, default: Optional[str] = None) -> Optional[str]:
    ...
    """
    if is_deployment_only_secret(name):
        return default
    ...
    if scope is not None:
        ...
        if val is not None:
            return val
        return default if (_MULTIPLEX_ACTIVE or isinstance(scope, SealedScope)) else _environ_or(name, default)
...
def build_profile_secret_scope(hermes_home: Path) -> Dict[str, str]:
    ...
    return SealedScope(secrets) if _profile_seals_credentials(Path(hermes_home)) else secrets


class SealedScope(dict):
    """A profile secret scope that must not fall through to ``os.environ``.

    Ordinary scopes are an *overlay*: when multiplexing is off and a key is
    missing, ``get_secret`` reads the process environment, because a
    single-profile deployment legitimately supplies credentials that way.

    That overlay is also a leak the moment one process serves several profiles
    without the multiplex flag — which is exactly what ``hermes serve`` does.
    Its environment carries whatever the deployment put there (on the owner's
    VPS: ``GITHUB_TOKEN``, ``GH_TOKEN``, ``COPILOT_GITHUB_TOKEN``,
    ``SENTRY_ACCESS_TOKEN``), so a profile with an empty ``.env`` resolves the
    deployment's GitHub token as if it were its own.

    A profile that opts out of that inheritance gets a sealed scope: a miss
    returns the default instead of the process environment. Deployment
    settings (``_is_global_env``) are unaffected — they are read before any
    scope is consulted and are not credentials.

    Sealing is per profile and off by default, so every existing profile
    behaves exactly as it did.
    """

    __slots__ = ()

def is_deployment_only_secret(name: str) -> bool:
    """Whether this name belongs to the deployment rather than to any agent."""
    return str(name or "").strip().upper() in _DEPLOYMENT_ONLY_SECRETS

def _profile_seals_credentials(home: Path) -> bool:
    """Whether this profile declines credentials from the process environment.

    Read straight from the profile's own ``config.yaml`` rather than the
    resolved config, because this runs inside credential resolution and must
    not re-enter it. Absent means False, so nothing that exists today changes
    behaviour.

    An agent created from Asera sets this: creating a teammate must not hand it
    the account access the deployment happens to hold.
    """
    try:
        # Diagnose the physical profile's seal via the canonical raw owner;
        # merged config would re-enter this credential-resolution bootstrap.
    ...
    if not isinstance(credentials, dict):
        return False
    return credentials.get("inherit_process_env") is False

_DEPLOYMENT_ONLY_SECRETS = frozenset({
