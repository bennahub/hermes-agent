...
from agent.execution_scope_preflight import ScopePreflightBlocked, blocked_result as scope_blocked_result
...
#: Durable sidecar key carrying the structural Connection descriptor on the failed turn's own
#: assistant row. ONE name for every surface: the live ``message.complete`` frame publishes the same
#: dict as ``payload["connection"]``, and both history projections re-publish it — read from this
#: key — as a top-level ``connection`` on the message. Clients read one key, live or cold.
CONNECTION_DISPLAY_METADATA_KEY = "connection"


def _append_connection_failure_row(agent, messages, conversation_history, final: str, block) -> None:
    """Give the failed turn a durable history row carrying the structural descriptor.

    Without this the turn persisted NOTHING for itself: the descriptor rode only the live
    ``message.complete`` frame and ``session.resume``'s inflight snapshot, both of which die with the
    process. After a relaunch the Owner saw their own prompt, no answer and no reconnect affordance —
    the turn looked like it had simply been swallowed.

    So the verdict becomes an ordinary assistant row: ``content`` is the owner-safe line (never the
    provider's prose, which stays in ``error``), and ``display_metadata['connection']`` carries the
    same descriptor the live frame does, structurally, so a cold launch and REST history rebuild the
    identical reconnect card instead of re-parsing text.

    Invariants held: the row is ``assistant``, so no second Owner bubble is authored and
    one-Owner-message-per-send is untouched. Fail-soft — a persistence problem must not turn a
    handled auth failure back into an unhandled crash.
    """
    if not isinstance(messages, list):
        return
    row: Dict[str, Any] = {"role": "assistant", "content": final, "timestamp": time.time()}
    if isinstance(block, dict) and block:
        row["display_metadata"] = {CONNECTION_DISPLAY_METADATA_KEY: dict(block)}
    messages.append(row)
    # Both terminal callers already flushed before building this result, so the append needs its own
    # flush. ``_persist_session`` dedupes on the intrinsic per-message marker, so re-running it here
    # writes exactly this row and nothing else.
    ...
    try:
        ...
        logger.warning("Failed to persist the connection-failure transcript row", exc_info=True)


def _connection_failure_result(
    *, classified, summary: str, messages, api_call_count: int, provider: str, base_url, model: str,
    status_code: Optional[int] = None, reason_code: str = "", agent=None, conversation_history=None,
) -> dict:
    """Structured terminal result for an auth-classified failure — the single construction point
    for the non-retryable abort and max-retries paths.

    ``summary`` (which carries the provider's own words, e.g. ``HTTP 401: OAuth access token has
    been revoked.``) is kept ONLY in ``error``, the diagnostic field. ``final_response`` — what the
    Owner actually reads — is built from the structural verdict, so a raw provider 401 can never be
    delivered as an ordinary agent answer again.

    Pass ``agent`` (both terminal callers do) so the verdict also lands in the durable transcript;
    see :func:`_append_connection_failure_row` for why a frame-only descriptor is not enough.
    """
    from agent.connection_health import connection_block_dict, needs_owner_summary, record_fault
    block = connection_block_dict(
        provider=provider, base_url=str(base_url or ""), reason_code=reason_code,
        message=summary, status_code=status_code,
    )
    # Publish to the canonical health store so the Connections screen stops reporting a broken
    # account as healthy (or as a static "external gate" with no reconnect action).
    record_fault(block)
    final = (block or {}).get("message") or (
        "A connection this request depends on is disconnected and needs to be reconnected."
    )
    _append_connection_failure_row(agent, messages, conversation_history, final, block)
    return {
        "final_response": final, "messages": messages, "api_calls": api_call_count,
        "completed": False, "failed": True,
        # Diagnostic only. Surfaces render ``final_response`` / ``connection_block``, never this.
        "error": summary,
        "failure_reason": classified.reason.value,
        # Owner-gated: a bare retry cannot clear a revoked grant, so no surface offers Retry.
        "failure_retryable": False,
        "connection_block": block,
        # Task semantics (#5D): the business task is NOT done — it is blocked on the Owner.
        "needs_owner": True,
        "needs_owner_reason": needs_owner_summary(block),
    }
...
def _run_conversation_with_authority(
    agent,
    user_message: Any,
    ...
    agent._execution_scope_stop = None
    ...
    except ScopePreflightBlocked as _scope_blocked:
        return scope_blocked_result(agent, _scope_blocked)
    ...
    if agent.api_mode == "codex_app_server":
        ...
        return {"final_response": "This runtime cannot mediate scoped tool execution. Use a Hermes-managed tool runtime.",
                "messages": s.messages, "failed": True,
                "error_type": "execution_scope_unsupported_runtime"}

    while (s.api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        from agent.execution_scope_outcomes import stop_message
        scope_stop = stop_message(agent)
        if scope_stop:
            s.failed = True
            s.final_response = scope_stop
            s._turn_exit_reason = agent._execution_scope_stop
            break
...
def run_conversation(agent, *args, **kwargs):
    """Execute one native turn; optional continuity failure cannot erase its result."""
    from agent.agent_runtime_helpers import clear_turn_authority
    from agent.autonomy.owner_continuity import finish_turn as finish_owner_continuity
    try:
        try:
            result = _run_conversation_with_authority(agent, *args, **kwargs)
        except Exception:
            try:
                finish_owner_continuity(agent, {"failed": True})
            except Exception:
                logger.warning("Owner responsibility remains pending after turn failure")
            raise
        try:
            finish_owner_continuity(agent, result)
        except Exception:
            logger.warning("Owner continuity deferred after successful native turn")
        return result
    finally:
        clear_turn_authority(agent)


run_conversation.__wrapped__ = _run_conversation_with_authority
