...
def _dispatch_authorized_once(
    ...
    *,
    ...
    from agent.execution_scope import ExecutionScopeDenied, execute_scoped
    from tools.arg_coercion import coerce_tool_args
    ref.args = coerce_tool_args(ref.name, ref.args)
    state.args = ref.args
    # Release batch ordering before a potentially slow policy judgment; actual
    # execution/admission instrumentation starts only inside the allowed callback.
    _advance_start_order()

    def admitted(args):
        from agent.autonomy.owner_continuity import prepare_dispatch
        prepare_dispatch(agent, ref.name, args)
        _begin_tool_execution(agent, ref, display_index)
        return _run_with_activity_heartbeat(agent, ref.name, lambda: execute(args))

    turn_id = getattr(agent, "_current_turn_id", None)
    try:
        result = execute_scoped(ref.name, ref.args, admitted,
                                turn_id=turn_id, invocation_id=ref.call_id)
    except Exception:
        from agent.execution_scope import get_binding, _blocked
        binding = get_binding(turn_id)
        if binding is None or not binding.db.get_scope_uncertainty(binding.scope_id, turn_id):
            raise
        result = _blocked("An admitted action ended without a confirmed outcome; inspect its effects before continuing", uncertain=True)
    if isinstance(result, ExecutionScopeDenied):
        state.blocked = not result.uncertain
    from agent.execution_scope_outcomes import record_gate_outcome
    record_gate_outcome(agent, result)
    return result
...
def _run_sequential_tool_execution_middleware(
    ...
    *,
    ...
    try:
        ...
        from agent.execution_scope_outcomes import abandon_attempt
        was_admitted = abandon_attempt(agent, ref.call_id)
        abandoned = True
        future.cancel()
        if state == "timeout":
            _interrupt_worker_tids(agent, worker_tid)
        if was_admitted is False:
            from agent.execution_scope import _blocked
            result = _blocked("Execution preparation timed out before admission; no action was started")
            ref.emit_post(agent, result, status="blocked", error_type="execution_scope_denied")
            return _ManagedToolResult(result=result, args=ref.args, middleware_trace=ref.trace,
...
def _commit_tool_result(
    ...
    *,
    ...
    if isinstance(function_result, ExecutionScopeDenied):
        blocked = not function_result.uncertain
        effect_disposition = "unknown" if function_result.uncertain else "none"
    elif blocked:
        effect_disposition = "none"
    try:
        from agent.autonomy.owner_continuity import observe_dispatch
        function_result = observe_dispatch(agent, function_name, function_args, function_result, failed=is_error, blocked=blocked)
    except Exception as exc:
        # The tool already ran. Preserve its canonical result and native handle;
        # a continuity-store error must never encourage replay of its effects.
        agent._owner_continuity_deferred = True
        logger.warning("Owner continuity deferred after tool result (%s)", type(exc).__name__)
    if observed:
        if not blocked:
    ...
    owner_work_id = getattr(agent, '_owner_continuity_work_id', None)
    if isinstance(owner_work_id, str) and owner_work_id:
        tool_message['display_metadata'] = {**(tool_message.get('display_metadata') or {}), 'owner_work_id': owner_work_id}
    if getattr(agent, '_owner_continuity_deferred', False) is True:
        tool_message['display_metadata'] = {**(tool_message.get('display_metadata') or {}), 'owner_continuity_deferred': True}
...
def _unfinished_tool_result(agent, ref: _ToolCallRef, *, timed_out: bool, timeout_s: float | None) -> tuple[str, float, Optional[str]]:
    ...
    if was_admitted is False:
        from agent.execution_scope import _blocked
        result = _blocked("Execution preparation ended before admission; no action was started")
        ref.emit_post(agent, result, status="blocked", error_type="execution_scope_denied")
