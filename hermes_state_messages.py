...
class SessionMessagesMixin:
    ...
    def _should_suppress_internal_background_message(
        self,
        session_id: str,
        role: Any,
        content: Any,
        ...
        """Drop only an exact no-op marker from hidden webhook/cron history."""
        ...
        try:
            ...
            if not is_intentional_silence_response(content):
                return False
            with self._read_ctx() as conn:
                row = conn.execute(
                    "SELECT source, hidden FROM sessions WHERE id = ?", (session_id,),
                ).fetchone()
            return bool(
                row
                and row["hidden"]
                and (row["source"] or "").strip().lower() in {"webhook", "cron"}
            )
        except Exception:
            logger.debug(
                "internal background no-op policy failed for %s",
                session_id,
                exc_info=True,
            )
            return False
    ...
    @staticmethod
    def _is_unambiguous_silence_control(
        role: Any,
        content: Any,
        tool_calls: Any = None,
    ) -> bool:
        """Whether an assistant row is a display-hidden control token."""
        if role != "assistant" or tool_calls:
            return False
        try:
            from gateway.response_filters import is_unambiguous_silence_control

            return bool(is_unambiguous_silence_control(content))
        except Exception:
            return False
    ...
    def append_message(
        ...
        if self._should_suppress_internal_background_message(
            session_id, role, content, tool_calls
        ):
            logger.info(
                "[background-delivery] suppressed internal no-op assistant message in session %s",
                session_id,
            )
            return -1

        # Preserve the assistant turn for provider role alternation while
        # hiding its control token from every presentation surface. api_content
        # is the byte-fidelity provider sidecar; content is what Desktop shows.
        hidden_silence_control = self._is_unambiguous_silence_control(
            role, content, tool_calls
        )
        if hidden_silence_control:
            try:
                from gateway.background_delivery import background_delivery_active

                # A single-row background append has no adjacent visible user
                # row to preserve; keep its historical drop behavior. The batch
                # path below can make the relational decision safely.
                if background_delivery_active():
                    hidden_silence_control = False
            except Exception:
                pass
        if hidden_silence_control:
            if not api_content:
        ...
        # A background event (webhook / routine / agent relay) runs a real chat
        # turn inside the canonical Bot Chat session, so its transport envelope
        # and a no-material-result reply would otherwise be persisted as owner
        # -facing rows. Suppress those here — the single persistence choke
        # point — so they never become a row, a preview, an unread, or a
        # notification. Transport filtering is activated by
        # HERMES_BACKGROUND_DELIVERY; exact silence control tokens are dropped
        # globally, including on interactive Desktop turns.
        # This is NOT Quiet Bot Mode (which hides detail on real messages).
        try:
            from gateway.background_delivery import should_suppress_owner_facing_message

            if (
                not hidden_silence_control
                and should_suppress_owner_facing_message(role, content, tool_calls)
            ):
                logger.info(
                    "[background-delivery] suppressed owner-facing %s message in session %s",
                    role,
                    session_id,
                )
                return -1
        except Exception:  # never break persistence on a policy error
            logger.debug("background delivery policy check failed", exc_info=True)
    ...
    def append_messages_batch(
        ...
        internal_kept = [
            m for m in messages
            if not self._should_suppress_internal_background_message(
                session_id,
                m.get("role"),
                m.get("content"),
                m.get("tool_calls"),
        ...
        if len(internal_kept) != len(messages):
            logger.info(
                "[background-delivery] suppressed %d internal no-op message(s) in session %s",
                len(messages) - len(internal_kept),
                session_id,
            )
            messages = internal_kept
            if not messages:
                return 0

        normalized_messages = []
        for message in messages:
            if self._is_unambiguous_silence_control(
                message.get("role"), message.get("content"), message.get("tool_calls")
            ):
                message = dict(message)
                if not message.get("api_content"):
                    message["api_content"] = message.get("content")
                message["content"] = ""
                if not message.get("display_kind"):
                    message["display_kind"] = "control"
            normalized_messages.append(message)
        messages = normalized_messages

        # ── Owner-facing background delivery policy ──────────────────────
        # The CLI chat lane (the one webhook / cron / agent-relay delivery
        # shells out to) flushes a whole turn through this batch writer, so
        # the policy must run here as well as in append_message. Filtering
        # BEFORE the transaction means a suppressed row never bumps
        # message_count, the latest preview, or the unread cursor.
        try:
            ...
            noncontrol_kept_indexes = {
                idx for idx, m in enumerate(messages)
                if (
                    m.get("display_kind") != "control"
                    and not should_suppress_owner_facing_message(
                        m.get("role"), m.get("content"), m.get("tool_calls")
                    )
                )
            }
            drop_controls = (
                background_delivery_active() and not noncontrol_kept_indexes
            )
            kept = [
                m for idx, m in enumerate(messages)
                if (
                    (m.get("display_kind") == "control" and not drop_controls)
                    or idx in noncontrol_kept_indexes
                )
            ]
            if len(kept) != len(messages):
                logger.info(
                    "[background-delivery] suppressed %d owner-facing message(s) "
                    "in session %s",
            ...
            logger.debug("background delivery batch policy failed", exc_info=True)
    ...
    def set_latest_matching_message_display_kind(
        self, session_id: str, *, role: str, content: str, display_kind: Optional[str] = None,
        ...
        during transcript rendering. Metadata without a kind merges onto an
        ordinary reply while preserving the row's existing presentation kind.
        """
        if not session_id or not content or not (display_kind or display_metadata):
            return False
        ...
        def _do(conn):
            ...
            if not display_kind:
                # Metadata-only replies must retain pre-existing annotations
                # and the row's kind; they are still ordinary owner bubbles.
                metadata = {**(self._decode_display_metadata(row[1]) or {}),
                            **(display_metadata or {})}
            conn.execute(
                "UPDATE messages SET display_kind = COALESCE(?, display_kind), display_metadata = ? WHERE id = ?",
                (
                    _scrub_surrogates(display_kind) if display_kind else None,
    ...
    #: Key under which published-file references live inside
    #: ``display_metadata``. The same sidecar reactions and structured replies
    #: use, so a client learns about a file the way it learns about a quote.
    ATTACHMENTS_METADATA_KEY = "attachments"

    def attach_artifacts_to_message(
        self,
        session_id: str,
        *,
        role: str = "assistant",
        ...
        """Merge published-file references onto this turn's persisted row.

        `finalize_turn` stamps the in-memory message, but the session flush is
        **append-only**: by the time the turn finalizes, the assistant row is
        usually already in the table, and mutating the dict afterwards never
        reaches SQLite. That is why the owner's phone saw an agent say "the file
        is attached" above a message carrying no attachment — the tool had run,
        the artifact was registered, and only the durable stamp was missing.

        Deliberately *not* `set_latest_matching_message_display_kind`: that one
        also writes ``display_kind``, and a typed runtime row is bookkeeping the
        transcript may hide from the owner. An attachment must never make the
        message carrying it disappear.

        Merges rather than replaces, like `set_message_reaction`, so a reply
        reference or a reaction already on the row survives.
        """
        if not session_id or not references:
            return False

        def _do(conn):
            # Always matched on content, the precedent's own approach — including
            # when the content is empty, which a tool-only or interrupted turn
            # legitimately produces. Falling back to "the newest assistant row"
            # would look right and be wrong: if this turn's row has not been
            # flushed yet, that row belongs to the *previous* turn, and the file
            # would be stamped onto a message the agent sent before it existed.
            # Stamping nothing is the safe miss — the in-memory dict still
            # carries the reference for anyone reading this turn live.
            row = conn.execute(
            ...
            existing = meta.get(self.ATTACHMENTS_METADATA_KEY)
            merged = list(existing) if isinstance(existing, list) else []
            known = {
                a.get("artifact_id") for a in merged if isinstance(a, dict)
            }
            for reference in references:
                if not isinstance(reference, dict):
                    continue
                # Idempotent: the in-memory stamp may already have reached the
                # row through a flush, and one file must not become two cards.
                if reference.get("artifact_id") in known:
                    continue
                known.add(reference.get("artifact_id"))
                merged.append(dict(reference))
            if not merged:
                return False

            meta[self.ATTACHMENTS_METADATA_KEY] = merged
            conn.execute(
    ...
    def get_message_for_reply(
        self, session_id: str, row_id: int
    ) -> Optional[Dict[str, Any]]:
        """The role and text of the active message at *row_id*, or ``None``.

        Scoped to *session_id* on purpose: a reply reference names a row, and a
        row id means nothing outside the conversation that owns it. Resolving
        it here is what stops a client quoting a message from somewhere else.
        """
        if not session_id:
            return None
