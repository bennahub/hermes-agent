...
def _(rid, params: dict) -> dict:
    ...
    activity_work_cache: dict = {}
    rows = [_session_live_item(sid, session, current, activity_work_cache=activity_work_cache)
            for sid, session in snapshot if not session.get("_finalized")]
...
def _(rid, params: dict) -> dict:
    ...
    sid = str(params.get("session_id") or "")
    # This RPC is an explicit Stop. The shared interrupt helper also handles
    # disconnected clients, which must retain durable Owner responsibility.
    continuity_status = "no_pending_work"
    try:
        from agent.autonomy.owner_continuity import cancel_owner_session
        durable_id = getattr(session.get("agent"), "session_id", None) or session.get("session_key")
        if isinstance(durable_id, str) and durable_id:
            if cancel_owner_session(durable_id, hermes_home=session.get("profile_home")):
                continuity_status = "stop_recorded"
    except Exception:
        continuity_status = "persistence_failed"
        logger.warning("Owner continuation cancellation could not be persisted; native Stop still applies")
    if _session_uses_compute_host(session):
        try:
            _interrupt_session_turn(sid, session, request_id=f"interrupt-{rid}")
        except Exception as exc:
            return _err(rid, 5019, f"compute-host interrupt failed: {exc}")
        return _ok(rid, {"status": "interrupted", "turn_isolation": True, "owner_continuity_status": continuity_status})
    session, err = _sess(params, rid)
    if err:
        return err
    ...
    return _ok(rid, {"status": "interrupted", "owner_continuity_status": continuity_status})
