...
class HostedRoomServerRPC:
    ...
    def _call(self, method: str, params: dict[str, Any], attachment_context=None) -> dict[str, Any]:
        rid = f"hosted-room-{next(self._ids)}"
        envelope = (self.server._submit_prompt(rid, params, attachment_context)
                    if attachment_context is not None and method == "prompt.submit"
                    else self.server._methods[method](rid, params))
    ...
    def submit(
        ...
        try:
            context = None
            if attachments:
                from tui_gateway.attachment_batches import BatchError, room_turn_context
                record = self._session_record(session_id)
                if record is None or not record.get("profile_home"):
                    raise HostedRoomSessionError("prompt.submit", 4004, "Room member profile is unavailable")
                try:
                    prompt, context = room_turn_context(self.server._hermes_home, record["profile_home"],
                                                         task.room_id, attachments, prompt)
                except (BatchError, OSError):
                    raise HostedRoomSessionError("prompt.submit", 4004, "Room attachment inputs are unavailable") from None
            return self._call("prompt.submit", {
                ...
                "_hosted_terminal_callback": on_terminal}, context)
