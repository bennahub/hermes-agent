...
def _enqueue_prompt(session: dict, text: Any, transport: Any, image_paths: list[str] | None = None,
                    ...
                    owner_task_nonce: str | None = None, client_message_id: str | None = None,
                    display_metadata: dict | None = None,
                    attachment_batch: dict | None = None) -> bool:
    ...
    # True means a new dispatchable envelope owns this nonce. False means
    # merge/drop: the caller must revoke the unused submission mint.
    ...
    text_only = (not attachment_batch and not image_paths and terminal_callback is None
                 and display_metadata is None and isinstance(text, str))
    # Never queue a text-only self-copy of the live prompt: draining it would restart it.
    if not client_message_id and text_only and text.strip() == _ac_inflight_original(session) != "":
        return False
    ...
    if display_metadata is not None:
        ...
        queued["display_kind"] = "a2a_message" if value and value["audience"] == "peer" else "hidden"
    if owner_task_nonce:
        queued["owner_task_nonce"] = owner_task_nonce
        from agent.message_projection import native_metadata
        queued["display_metadata"] = native_metadata(
            "owner", "owner", "message", metadata=queued.get("display_metadata"),
            source_session_id=session.get("session_key"))
    if client_message_id:
        queued.setdefault("display_metadata", {})["client_message_ids"] = [client_message_id]
    if attachment_batch:
        queued["image_paths"] = list(attachment_batch["image_paths"])
        queued["persist_text"] = attachment_batch["caption"]
        queued.setdefault("display_metadata", {}).update(attachment_batch["metadata"])
    if terminal_callback is not None:
        queued["terminal_callback"] = terminal_callback
    existing = session.get("queued_prompt")
    if (existing and existing.get("terminal_callback") is None and not existing.get("display_kind")
            and text_only and isinstance(existing.get("text"), str)
            and not existing.get("image_paths") and not (existing.get("display_metadata") or {}).get("attachment_batch_id")
            and not session.get("queued_prompts")):
        prev = existing["text"]
        existing["text"] = f"{prev}\n\n{text}" if prev and text else (prev or text)
        if client_message_id:
            metadata = existing.setdefault("display_metadata", {})
            ids = metadata.setdefault("client_message_ids", [])
            if client_message_id not in ids:
...
def _sanitize_queued_entry_vs_inflight_user(entry: Any, original: str) -> dict | None:
    ...
    if not isinstance(entry, dict):
        return None
    # Identity-bearing sends are distinct Owner submissions even when their words repeat.
    if entry.get("display_kind") or (entry.get("display_metadata") or {}).get("client_message_ids"):
        return entry
    text = entry.get("text")
    if not original or entry.get("terminal_callback") is not None or entry.get("image_paths") or not isinstance(text, str):
        return entry
...
def _handle_busy_submit(rid, sid: str, session: dict, text: Any, transport: Any, queued: bool = False,
                        owner_task_nonce: str | None = None, client_message_id: str | None = None,
                        attachment_batch: dict | None = None) -> dict | None:
    ...
    with session["history_lock"]:
        ...
        image_paths = list(attachment_batch["image_paths"] if attachment_batch else session.get("attached_images", []))
        if image_paths and attachment_batch is None:
    ...
    with session["history_lock"]:
        if not session.get("running"):
            ...
            return None
        queued_new_envelope = _enqueue_prompt(session, text, transport, image_paths=image_paths,
                                              owner_task_nonce=owner_task_nonce, client_message_id=client_message_id,
    ...
    if mode == "interrupt" and not image_paths and attachment_batch is None:
        _interrupt_busy_session(sid, session, agent)
    return _ok(rid, {"status": "queued", "queued_new_envelope": bool(queued_new_envelope),
                     "client_message_id_supported": not _session_uses_compute_host(session)})
...
def _drain_queued_prompt(rid, sid: str, session: dict) -> bool:
    ...
    with session["history_lock"]:
        if int(session.get("_queued_prompt_generation", 0)) != queue_generation:
            ...
            cancelled_callback = queued.get("terminal_callback")
    ...
    if cancelled_callback is not None:
        cancelled_callback({"status": "failed", "error": "queued prompt was cancelled before it could start"})
    if claim_cancelled:
        return True
    kwargs: dict = {"queued_prompt_generation": queue_generation}
    if "image_paths" in queued:
        kwargs["image_paths"] = queued["image_paths"]
    if "persist_text" in queued:
        kwargs["persist_text"] = queued["persist_text"]
    dispatch_failed = False
    try:
        if not use_compute_host:
            callback = queued.get("terminal_callback")
            if callback is not None:
                kwargs["terminal_callback"] = callback
            if queued.get("owner_task_nonce"):
                kwargs["owner_task_nonce"] = queued["owner_task_nonce"]
            ...
            if accepted is False:
                ...
                if callback is not None:
                    callback({"status": "failed", "error": "queued prompt was cancelled before it could start"})
...
def _inflight_snapshot(session: dict) -> dict | None:
    ...
    if error:
        ...
        # A broken Connection outlives the socket that reported it: replay the descriptor so the
        # resuming client rebuilds the reconnect card instead of rendering the raw provider error.
        if isinstance(connection := turn.get("connection"), dict) and connection:
            snapshot["connection"] = connection
    return snapshot


def _resolve_terminal_connection_block(
    session: dict, error: Any, error_surface: Optional[dict], connection: Optional[dict]) -> Optional[dict]:
    """The Connection descriptor this terminal error frame must carry, or None.

    Three sources, most-specific first: an explicit descriptor from the caller; the one already
    retained on the in-flight turn (a previous ``_fail_inflight_turn`` stored it, and this path used
    to erase it by passing ``connection=None``); and finally the escaping exception itself.

    The third source is the one that mattered: an auth failure that escapes as an exception never
    reaches ``_connection_failure_result``, so nothing built a descriptor and nothing recorded the
    fault. The frame then carried the provider's own 401 prose as the turn's text, under a Retry the
    Owner could press forever.

    Detection reuses the surface descriptor already computed above rather than re-classifying: it
    runs the same ``classify_api_error`` taxonomy behind the same "does this even look like an API
    exception" gate, so a gateway bug whose message happens to contain ``401`` cannot be promoted
    into a fake Connection fault. Fail-soft: no descriptor means the old generic error frame.
    """
    if isinstance(connection, dict) and connection:
        return dict(connection)
    retained = (session.get("inflight_turn") or {}).get("connection")
    if isinstance(retained, dict) and retained:
        return dict(retained)
    if not isinstance(error, BaseException) or not isinstance(error_surface, dict):
        return None
    agent = session.get("agent")
    try:
        from agent.connection_health import connection_block_dict, record_fault
        from agent.error_surface import LAYER_AUTH
        if error_surface.get("layer") != LAYER_AUTH:
            return None
        block = connection_block_dict(
            provider=str(getattr(agent, "provider", "") or ""),
            base_url=str(getattr(agent, "base_url", "") or ""),
            message=str(error), status_code=getattr(error, "status_code", None),
        )
    except Exception:
        logger.debug("terminal turn error: connection classification failed", exc_info=True)
        return None
    # An observation is an observation however the turn died: record it so Connections stops
    # reporting the account healthy just because the failure took the exception path.
    with contextlib.suppress(Exception):
        record_fault(block)
    return block
...
def _emit_terminal_turn_error(
    ...
    retire_marker: bool = True, connection: Optional[dict] = None) -> None:
    ...
    A Connection fault rides this frame exactly as it rides the returned-error one
    (:func:`_resolve_terminal_connection_block`): the same structural ``connection`` descriptor, the
    owner-safe sentence instead of the provider's prose, and ``retryable: False`` pinned on the
    surface — a Retry button on an Owner-gated fault is a button that cannot work."""
    agent = session.get("agent")
    ...
    with session["history_lock"]:
        block = _resolve_terminal_connection_block(session, error, error_surface, connection)
        if block and isinstance(error_surface, dict):
            # The classifier calls a 401 retryable because Hermes may rotate to another credential.
            # By the time the turn is terminal that has already been tried and lost, so the client
            # must not be told a bare retry can work. Matches ``_connection_failure_result``. Pinned
            # BEFORE the retain, so ``session.resume`` replays the corrected verdict too.
            error_surface = {**error_surface, "retryable": False}
        _fail_inflight_turn(session, error, error_surface=error_surface, connection=block)
    ...
    # Never the provider's own words: an owner-safe sentence built from the structural verdict.
    text = partial or (block or {}).get("message") or f"Error: {message}"
    rendered = ""
    with contextlib.suppress(Exception):
    ...
    if block:
        # Same four keys the returned-error path publishes, so a client has one branch for a broken
        # Connection whether the turn ended by returning a result or by raising.
        payload["connection"] = block
        payload["failure_reason"] = (
            error_surface.get("code") if isinstance(error_surface, dict)
            and error_surface.get("layer") == "auth" and error_surface.get("code") else "auth")
        payload["needs_owner"] = True
        payload["failure_retryable"] = False
    if retire_marker:
