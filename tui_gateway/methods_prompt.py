...
def _run_after_agent_ready(rid, sid, session, text, display_kind, hosted_terminal_callback,
                           owner_task_nonce=None, reply_reference=None, client_message_id=None,
                           received_monotonic=None, attachment_batch=None):
    """Turn thread body: patient wait for a deferred build (a slow build must not eat the
    accepted in-flight message), then run."""
    prior_run_thread = session.get("_run_thread")
    progress_watch = None
    if owner_task_nonce:
        try:
            from tui_gateway.interactive_progress import OwnerProgressWatch
            progress_watch = OwnerProgressWatch(
                session, lambda event, payload: _emit(event, sid, payload),
                lambda: _session_live_status(sid, session) == "waiting",
                received=received_monotonic, ui_session_id=sid,
                client_message_id=client_message_id)
        except Exception:
            logger.debug("interactive progress observer unavailable", exc_info=True)
    try:
        ...
        if err:
            ...
            if hosted_terminal_callback is not None:
                hosted_terminal_callback({"status": "failed", "text": "",
                                          "error": (err.get("error") or {}).get("message", "agent initialization failed")})
        ...
        if progress_watch is not None:
            progress_watch.mark_agent_ready()
            progress_watch.bind_agent(session.get("agent"))
        with session["history_lock"]:
            if session.get("_turn_cancel_requested") or not session.get("running"):
                _revoke_prompt_owner_nonce(owner_task_nonce)
                if hosted_terminal_callback is not None:
                    hosted_terminal_callback({"status": "cancelled", "text": "",
                                              "error": "Turn cancelled before the agent was ready"})
                session["running"] = False
                _clear_inflight_turn(session)
                ...
                return
        composed, reply_to, reply_quote = _compose_owner_turn_text(session, text, reply_reference)
        metadata = {"reply_to": reply_to} if reply_to else {}
        from agent.message_projection import native_metadata
        metadata = native_metadata(
            "runtime" if display_kind else "owner",
            "internal" if display_kind else "owner",
            "control" if display_kind else "message",
            metadata=metadata, source_session_id=session.get("session_key"))
        if client_message_id:
            metadata["client_message_ids"] = [client_message_id]
        if attachment_batch:
            metadata.update(attachment_batch["metadata"])
        started = _run_prompt_submit(
            rid, sid, session, composed,
            display_kind=display_kind,
            display_metadata=metadata or None,
            persist_text=attachment_batch["caption"] if attachment_batch else text if reply_quote else None,
            image_paths=attachment_batch["image_paths"] if attachment_batch else None,
            terminal_callback=hosted_terminal_callback,
            owner_task_nonce=owner_task_nonce,
            progress_watch=progress_watch)
        if started:
            # _run_prompt_submit returns on admission, before its daemon worker
            # finishes. That worker now owns the watch through real turn cleanup.
            progress_watch = None
        if not started and hosted_terminal_callback is not None:
            hosted_terminal_callback({"status": "failed", "text": "", "error": "Turn dispatch refused"})
    except BaseException:
        run_thread = session.get("_run_thread")
        worker_started = (run_thread is not prior_run_thread
                          and getattr(run_thread, "ident", None) is not None)
        if not worker_started and (progress_watch is None or not progress_watch.worker_owned):
            _revoke_prompt_owner_nonce(owner_task_nonce)
            with session["history_lock"]:
                session["running"] = False
                session["last_active"] = time.time()
        ...
        if progress_watch is not None and not progress_watch.worker_owned:
...
def _lock_in_submit_turn(
    ...
    with session["history_lock"]:
        ...
        _start_inflight_turn(
            session, text, display_kind="hidden" if params.get("display_kind") == "hidden" else None)
    return None, fields


@method("prompt.submit")
def _(rid, params: dict) -> dict:
    batch = params.get("attachment_batch")
    if batch is None:
        return _submit_prompt(rid, params)
    from tui_gateway import attachment_batches
    import sqlite3
    home = key = message_id = None
    try:
        if not isinstance(batch, dict):
            raise attachment_batches.BatchError("attachment_batch must be an object")
        message_id = attachment_batches.identity(params.get("client_message_id"))
        if attachment_batches.identity(batch.get("batch_id")) != message_id:
            raise attachment_batches.BatchError("batch_id must equal client_message_id")
        if not isinstance(params.get("text", ""), str):
            raise attachment_batches.BatchError("Attachment caption must be text")
        if any(params.get(k) is not None for k in _TRUNCATION_PARAMS) or params.get("display_kind") or params.get("_hosted_task"):
            raise attachment_batches.BatchError("Attachment batches require an ordinary owner turn")
        session, err = _sess_nowait(params, rid)
        if err:
            return err
        if _session_uses_compute_host(session, _load_dashboard_process_isolation_config()):
            return _err(rid, 4121, "Attachment batches require the native inline session runtime")
        home = _session_home_dir(session, "")
        key = str(session.get("session_key") or "")
        with _session_db(session) as db:
            # Native lease identity follows compression only, never delegate or
            # explicit branch children (get_conversation_root is broader).
            key = db._session_turn_lease_key(key) if db is not None else key
        profile = _session_info(session.get("agent"), session).get("profile_name") or "default"
        staged = attachment_batches.stage(
            home, profile, key, message_id, message_id, batch.get("items"))
        from hermes_cli.input_sanitize import sanitize_user_prompt_text
        caption = sanitize_user_prompt_text(params.get("text", ""))
        transcript = None
        if batch.get("transcript") is not None:
            from agent.message_projection import canonical_transcript
            submitted_transcript = batch.get("transcript")
            # Before staging the client can know only its stable item identity;
            # Hermes owns artifact identity and derives it from the staged row.
            audio_item_id = (submitted_transcript.get("audio_item_id")
                             if isinstance(submitted_transcript, dict) else None)
            transcript = canonical_transcript(submitted_transcript, staged["attachments"])
            if (not isinstance(audio_item_id, str) or not audio_item_id
                    or transcript is None or transcript["text"] != caption.strip()):
                raise attachment_batches.BatchError(
                    "Transcript must name an audio attachment and match the submitted caption")

        def dispatch(records, paths):
            # The ordered manifest is model input only. The owner row persists
            # caption plus artifact IDs, never these private filesystem refs.
            lines = [caption, "", "Attachments (in supplied order):"]
            for index, (record, path) in enumerate(zip(records, paths), 1):
                ref_kind = "image" if record["kind"] == "image" else "file"
                lines.append(f"{index}. {record['filename']} @{ref_kind}:{_format_ref_value(path)}")
            metadata = {"attachments": records, "attachment_batch_id": message_id}
            if transcript is not None:
                metadata["transcript"] = transcript
            context = {"caption": caption,
                       "image_paths": [p for r, p in zip(records, paths) if r["kind"] == "image"],
                       "metadata": metadata}
            return _submit_prompt(rid, {**params, "text": "\n".join(lines), "queued": True}, context)

        result = attachment_batches.submit(home, key, message_id, message_id,
                    {"text": caption, "reply_to": params.get("reply_to"),
                     "transcript": transcript}, dispatch)
        return result if result.get("error") else _ok(rid, result)
    except attachment_batches.BatchError as exc:
        proof = exc if isinstance(exc, attachment_batches.RefusedBatch) else None
        if proof is None and home is not None and key and message_id:
            try:
                # The seal carries the reason, so the identity keeps answering
                # with it. Without this the Owner is told only that a message
                # "was not admitted" and has nothing to change.
                proof = attachment_batches.seal_refusal(
                    home, key, message_id, message_id, attachment_batches.refusal_of(exc))
            except (attachment_batches.BatchError, OSError, sqlite3.Error):
                pass  # No durable seal, no permission to reconfigure.
        return _err(rid, 4004, str(proof or exc), proof.data if proof else None)
    except (OSError, sqlite3.Error):
        return _err(rid, 5071, "Attachment storage could not be written; retry with the same message identity")


def _submit_prompt(rid, params: dict, attachment_batch=None) -> dict:
    ...
    if client_message_id is not None:
        ...
        try:
            ...
            return _err(rid, 4004, "client_message_id must be a UUID")
    ...
    _minted_authority_nonce: dict[str, str | None] = {"nonce": None}

    def _mint_owner_task_authority(reason: str) -> str | None:
        """Records that an authenticated owner task was accepted, returns its nonce.

        Called only where the prompt is actually taken — queued, or starting —
        never merely received, and never for a hidden continuation. The nonce it
        returns is kept so a path that mints but then does not start a turn can
        revoke exactly that grant (`_revoke_owner_task_authority`).
        """
        if internal_hosted_submit or display_kind == "hidden":
            return None
        try:
            from tools import owner_task_authority as _owner_authority
            nonce = _owner_authority.mint_pending(
                [
                ...
                source=reason,
                instruction=attachment_batch["caption"] if attachment_batch else (
                    _compose_owner_turn_text(session, text, params.get("reply_to"))[0]
                    if isinstance(text, str) else text
                ),
                source_id=(f"rpc:{sid}:{client_message_id}" if client_message_id else None),
            )
            _minted_authority_nonce["nonce"] = nonce
            return nonce
        except Exception:
            logger.debug("owner-origin mint skipped", exc_info=True)
            return None

    def _revoke_owner_task_authority() -> None:
        """Drop this submit's mint when it did not go on to start a turn."""
        nonce = _minted_authority_nonce.get("nonce")
        if not nonce:
            return
        _minted_authority_nonce["nonce"] = None
        try:
            from tools import owner_task_authority as _owner_authority
            _owner_authority.revoke_pending(nonce)
        except Exception:
            logger.debug("owner-origin revoke skipped", exc_info=True)
    err = (
    ...
    while True:
        ...
        # Mint BEFORE the busy handler runs, so the nonce is attached to the
        # queue envelope AS it is created, under the same lock — no second,
        # racy "stamp the newest envelope" pass a concurrent submit or a drain
        # could win. The mint is revoked below for anything that is not a clean
        # new queue: an error, a steer/redirect into the running turn, a
        # merge/drop, or the fall-through where the turn ended between locks.
        _mint_owner_task_authority("gateway.prompt.submit.queued")
        from agent.execution_scope import begin_amendment, cancel_amendment
        _amendment_source = "amendment:" + (_minted_authority_nonce.get("nonce") or "")
        _amendment_token = (begin_amendment(session.get("agent"), _amendment_source,
                                             text=attachment_batch["caption"] if attachment_batch else text)
                            if _minted_authority_nonce.get("nonce") else None)
        try:
            busy_response = _handle_busy_submit(
                rid, sid, session, text, busy_transport,
                queued=bool(params.get("queued")),
                owner_task_nonce=_minted_authority_nonce.get("nonce"),
                client_message_id=client_message_id,
                attachment_batch=attachment_batch)
        except BaseException:
            cancel_amendment(_amendment_token)
            _revoke_owner_task_authority()
            raise
        if busy_response is not None:
            # A message steered or redirected INTO the running turn joins a turn
            # that already bound its own grant (or has none); a merged/dropped
            # text submission starts no turn of its own. In each of those the
            # nonce was never attached to a dispatchable envelope, so the mint is
            # a leftover — revoke it. An error is not an acceptance at all.
            _status = busy_response.get("status") if isinstance(busy_response, dict) else None
            _result_status = (
                (busy_response.get("result") or {}).get("status")
                if isinstance(busy_response, dict) else None
            )
            _joined_running_turn = "steered" in {_status, _result_status} or \
                "redirected" in {_status, _result_status}
            if _joined_running_turn and _amendment_token is not None:
                from tui_gateway.prompt_authority import start_owner_amendment
                start_owner_amendment(session.get("agent"),
                    attachment_batch["caption"] if attachment_batch else text,
                    _amendment_source, _amendment_token)
            else:
                cancel_amendment(_amendment_token)
            # The busy handler reports a genuine new envelope as
            # queued_new_envelope; strip that internal flag from the client
            # response and use it to decide whether the mint has a home.
            _made_new_envelope = True
            if isinstance(busy_response, dict):
                _res = busy_response.get("result")
                if isinstance(_res, dict) and "queued_new_envelope" in _res:
                    _made_new_envelope = bool(_res.pop("queued_new_envelope"))
            if (isinstance(busy_response, dict) and busy_response.get("error")) \
                    or _joined_running_turn or not _made_new_envelope:
                _revoke_owner_task_authority()
            return busy_response
        cancel_amendment(_amendment_token)
        # The old turn finished between the two lock acquisitions. Drop the mint
        # the busy path made and retry the claim, so this prompt starts normally
        # (and mints again there) instead of being stranded in a queue whose
        # drain already ran.
        _revoke_owner_task_authority()
    ...
    if (err := _persist_session_row_for_submit(rid, session)) is not None:
        return err
    # Mint only after all durable admission checks; compute-host dispatch owns its ingress.
    _mint_owner_task_authority("gateway.prompt.submit")
    try:
        ...
        run_thread = threading.Thread(
            target=lambda: _run_after_agent_ready(
                ...
                _minted_authority_nonce.get("nonce"), params.get("reply_to"), client_message_id,
                received_monotonic, attachment_batch),
            daemon=True)
        ...
        with session["history_lock"]:
            session["running"] = False
        raise
    return _ok(rid, {"status": "streaming", "client_message_id_supported": True, **survivor_fields})


# ── attachments ─────────────────────────────────────────────────────────────

@method("attachments.capabilities")
def _(rid, params: dict) -> dict:
    """Additive feature negotiation: old servers must not receive batch submits."""
    from tui_gateway import attachment_batches
    # What this build can actually decode, so a client converts at the picker
    # instead of uploading a photo admission would refuse. Omitted rather than
    # empty when this build cannot answer: [] would read as "no image ever".
    readable = attachment_batches.supported_image_mime_types()
    return _ok(rid, {"request_bound_batches": 1, "direct": True, "groups_local": True,
                     "session_modes": ["inline"], "groups_peer": False,
                     "max_items": attachment_batches.MAX_ITEMS,
                     "max_item_bytes": attachment_batches.MAX_BYTES,
                     "max_batch_bytes": attachment_batches.MAX_BATCH_BYTES,
                     **({"image_mime_types": readable} if readable else {})})
