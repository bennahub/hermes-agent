...
def _profiles_sig():
    """(live named-profile ids, tombstone mtime) — the roster's shape, stat-cheap.

    The roster is changed by OTHER processes at least as often as by this gateway: a shell
    running ``hermes profile delete``, the dashboard's DELETE, or the same owner's second
    device calling ``profiles.delete`` on another transport. The profiles directory is the
    only signal all of them move, exactly like the pairing ledgers. The tombstone dir is
    folded in so a delete-then-recreate of the same name still registers as a change.

    The names come from the roster's OWN filter (``named_profile_dirs_in``), not a raw listing.
    A raw listing counts a tombstoned name as live, so any stale ``mkdir`` under a deleted
    profile — a logging or serve path that has not noticed the delete yet — moved this
    signature and fired ``profiles.changed`` at every device for a profile the roster does not
    show. Deletes still register: the name leaves the live set and the tombstone mtime moves.
    """
    home = _watcher_home()
    root = home.parent.parent if home.parent.name == "profiles" else home
    profiles_dir = root / "profiles"
    names: list[str] = []
    with contextlib.suppress(Exception):  # import too: the watcher must never raise
        from hermes_cli.profiles import named_profile_dirs_in
        names = [entry.name for entry in named_profile_dirs_in(profiles_dir)]
    return tuple(names), _watcher_mtime_ns(profiles_dir / ".deleted")
...
_CHANGE_WATCHES: dict[str, tuple[float, Any, Any]] = {
    ...
    # A deleted or created agent must leave/enter every connected client's roster without a
    # relaunch; profiles.delete also broadcasts directly so the owner's own device is instant.
    "profiles.changed": (2.0, _profiles_sig, lambda: {}),
