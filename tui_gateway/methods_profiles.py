...
def _latest_message_preview(db, session_id):
    """(≤80-char excerpt, role) of newest visible active user/assistant text, or ("", "").
    Uses the same native owner-audience predicate as unread and push."""
    from agent.message_projection import owner_preview
    row = None
    try:
        with db._lock:
            rows = db._conn.execute(
                "SELECT content, role, display_kind, display_metadata FROM messages"
                " WHERE session_id = ? AND role IN ('user', 'assistant')"
                " AND active = 1"
                " AND (role = 'user' OR COALESCE(json_array_length(CASE WHEN json_valid(tool_calls) THEN tool_calls ELSE '[]' END), 0) = 0)"
                " AND content IS NOT NULL AND TRIM(content) != ''"
                " ORDER BY id DESC", (session_id,))
            row = next((r for r in rows if owner_preview(r[1], r[0], r[2], r[3])), None)
    except Exception:
        return "", ""
    text = " ".join(str(row[0] or "").split()).strip() if row else ""
    return (text[:80] + "..." if len(text) > 80 else text), (str(row[1] or "") if row else "")


def _canonical_owner_read_state(db, root_id):
    """Native read watermark plus actual owner-directed reply activity.

    Session activity includes tools and heartbeat bookkeeping; it cannot be a
    message cursor. Use the canonical compression lineage's persisted assistant
    rows, excluding display-typed runtime rows and the bare silence sentinel.
    Internal incoming traffic is authored as user; assistant prose is not
    reclassified by a phrase such as "Message from the bank". Do not
    inspect JSON/prose shape: a requested JSON answer is still a real reply.
    No state is written here and no new read store is introduced.
    """
    from agent.message_projection import owner_attention
    lineage = db.get_compression_chain(root_id)
    placeholders = ",".join("?" for _ in lineage)
    with db._lock:
        rows = db._conn.execute(
            "SELECT timestamp, content, display_kind, display_metadata FROM messages WHERE session_id IN (" + placeholders + ")"
            " AND role = 'assistant' AND active = 1"
            " AND COALESCE(json_array_length(CASE WHEN json_valid(tool_calls) THEN tool_calls ELSE '[]' END), 0) = 0"
            " AND (tool_name IS NULL OR tool_call_id IS NULL)"
            " AND (TRIM(COALESCE(content, '')) != '' OR EXISTS ("
            " SELECT 1 FROM json_each(CASE WHEN json_valid(display_metadata) THEN display_metadata ELSE '{}' END, '$.attachments') a"
            " WHERE a.type = 'object' AND json_type(a.value, '$.artifact_id') = 'text'"
            " AND LENGTH(json_extract(a.value, '$.artifact_id')) > 0))"
            " AND TRIM(COALESCE(content, '')) != '[SILENT]' ORDER BY timestamp DESC",
            lineage,
        )
        latest = next((r[0] for r in rows if owner_attention('assistant', r[1], r[2], r[3])), None)
        # A continuation created after its parent was read starts with NULL.
        # Inherit the lineage's proven read rather than resurrecting old replies.
        read_at = db._conn.execute(
            "SELECT MAX(last_read_at) FROM sessions WHERE id IN (" + placeholders + ")", lineage
        ).fetchone()[0]
    return {"version": 1, "last_read_at": read_at, "latest_reply_at": latest}
...
def _canonical_session_row(db, profile_path):
    ...
    try:
        ...
        preview, preview_role = _latest_message_preview(db, tip)
        return {
            "id": session_id, "resolved_id": tip, "root_title": row.get("title") or "",
            "title": tip_row.get("title") or "", "preview": preview, "preview_role": preview_role,
            ...
            "owner_read_state": _canonical_owner_read_state(db, session_id)}
    except Exception:
        return None
    ...
    try:
        ...
        for s in db.list_sessions_rich(source=None, limit=20, order_by_last_active=True, compact_rows=True):
            ...
            if _denied_source(s):
                ...
                preview, preview_role = _latest_message_preview(db, s["id"])
                human = {"id": s["id"], "title": title,
                         "preview": preview or s.get("preview") or "", "preview_role": preview_role,
...
def _seal_profile_credentials(profile_dir) -> None:
    """Record that this profile does not inherit process-environment secrets.

    Written into the profile's own config.yaml, which is the file
    ``agent.secret_scope`` reads when it builds the scope for a turn.
    """
    ...
    if not isinstance(credentials, dict):
        credentials = {}
    credentials["inherit_process_env"] = False
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        path = profiles_mod.create_profile(
            ...
            inherit_model_credentials=is_truthy_value(params.get("mirror_credentials", True)))
    ...
    credentials_sealed = (is_truthy_value(params.get("seal_credentials", False))
                          and _best_effort(lambda: _seal_profile_credentials(path)))
    ...
    return _ok(rid, {"ok": True, "name": name, "path": str(path), "soul_written": soul_written,
                     "model_set": model_set, "credentials_sealed": credentials_sealed, "mirrored": mirrored})


@method("profiles.delete")
def _(rid, params: dict) -> dict:
    """Delete a profile (ws twin of DELETE /api/profiles/<name>). Params: ``name``.

    This handler adds NO deletion semantics of its own — a second teardown would drift from
    the CLI and the dashboard. It calls the one canonical lifecycle, which closes the A2A
    peer-room fence, retires the agent's logical Computer, disables the service, stops the
    gateway and any stray profile backends, tombstones the home so a live ``serve`` cannot
    mkdir it back into the roster, removes the alias wrapper and the directory, and resets
    ``active_profile``. The owner's confirmation is collected by the client, so the
    interactive prompt is always skipped — exactly what the dashboard's DELETE does.

    The broadcast is what makes the delete land on the owner's OTHER devices: every live
    transport drops the row without waiting for the filesystem watcher's next tick.

    **No server filesystem path leaves here — not in the result, not in an error.** The
    canonical lifecycle is a CLI first, so it says what an operator standing at a shell needs:
    ``delete_profile``'s ``rmtree`` failure names the directory it could not remove and the raw
    OS error, and its refusal to delete the default names ``~/.hermes`` and tells you to run
    ``hermes uninstall``. Both are right there and wrong here — the Owner is holding a phone,
    cannot act on either, and a server path on the wire is a disclosure whatever the client
    does with it. So the operator-facing detail is logged where operators read, and the wire
    gets the same facts an Owner can actually use: which agent, what state it is in, and where
    the rest of it went. ``profiles.create`` and ``profiles.list`` still return ``path``; those predate this
    handler and are the dashboard's contract, so they are not this handler's to change.
    """
    name = str(params.get("name") or "").strip()
    if not name:
        return _err(rid, 4067, "name required")
    # Checked here rather than read back out of the CLI's refusal: the message that refusal
    # carries is shell prose about ``~/.hermes`` and ``hermes uninstall``, and string-matching
    # it to decide what to send instead would put this handler one wording change away from
    # leaking it again.
    if _try(lambda: _lazy("hermes_cli.profiles", "normalize_profile_name")(name), name) == "default":
        return _err(rid, 4068, "The default agent cannot be deleted.")
    try:
        from hermes_cli import profiles as profiles_mod
        profiles_mod.delete_profile(name, yes=True)
    except (ValueError, FileNotFoundError) as e:
        # An invalid or unknown name is a client error, and both of those messages name only
        # the name the client sent.
        return _err(rid, 4068, str(e))
    except Exception:
        # Where the operator-facing detail goes. `delete_profile` already printed the state it
        # left behind and the remedy; this is the same information reaching the same audience
        # through the gateway, and it is the reason the wire does not need it.
        logger.warning("profiles.delete failed for %r", name, exc_info=True)
        return _err(rid, 5067,
                    f"Deleting '{name}' did not finish. Some of it may still be on the server, "
                    f"and the name may be refused until an operator clears it. The detail is in "
                    f"the server log.")
    _best_effort(lambda: _broadcast_global_event("profiles.changed", {"deleted": name}))
