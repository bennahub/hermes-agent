    ...
    try:
        with _session_profile_runtime_scope(session):
            if home:
                ...
                session_db = _open_profile_session_db(home)
                owns_db = True
        ...
        for name in ("background_review_callback", "memory_notifications"):
            if hasattr(agent, name):
                setattr(new_agent, name, getattr(agent, name))
        if owns_db and not _transfer_db_to_agent(new_agent, session_db):
            raise RuntimeError("capability rebuild did not retain the profile database")
        session["agent"] = new_agent
        owns_db = False
        session["config_model_seen"] = config_model_seen
        # Retire only the old database reference. Closing the whole agent would
        # end this SAME conversation and stop its still-owned task resources.
        if getattr(agent, "_owns_session_db", False):
            from hermes_state import release_or_close

            old_db = getattr(agent, "_session_db", None)
            agent._owns_session_db = False
            if old_db is not None and old_db is not _get_db():
                release_or_close(old_db)
        _emit(
            "notice",
            sid,
