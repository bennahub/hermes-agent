...
def _history_to_messages(history: list[dict]) -> list[dict]:
    from agent.message_projection import (
        canonical_attachments, compact_owner_collection, connection_fault, owner_safe_metadata,
        owner_semantics, projection, timeline_visible,
    )
    messages = []
    tool_call_args = {}
    for m in compact_owner_collection(history):
        ...
        if not timeline_visible(role, content_text, m.get("display_kind"), m.get("display_metadata")):
            continue
        ...
        if role == "tool":
            ...
            if name == "request_connection":
                from tools.connection_request_tool import normalize_connection_request
                projected = normalize_connection_request(m.get("content"))
                if projected is not None:
                    tool_msg["result"] = projected
                    tool_msg["tool_id"] = m.get("tool_call_id", "")
            messages.append(tool_msg)
            continue
        # A reasoning-only assistant turn is kept so "Thinking…" still shows after resume/reload.
        # An artifact-only Owner turn is also a complete message: its attachment
        # card is the content even when its optional caption is empty.
        has_reasoning = role == "assistant" and any(m.get(key) for key in _HISTORY_REASONING_KEYS)
        has_attachments = bool(canonical_attachments(m.get("display_metadata")))
        has_collaboration = isinstance(m.get("collaboration_summary"), dict)
        if not content_text.strip() and not has_reasoning and not has_attachments and not has_collaboration:
            continue
        msg = {"role": role, "text": content_text}
        if has_collaboration:
            msg["collaboration_summary"] = m["collaboration_summary"]
        ...
        if m.get("display_metadata"):
            msg["display_metadata"] = owner_safe_metadata(m["display_metadata"])
        # A turn that died on a broken Connection republishes its structural descriptor under the
        # SAME key the live `message.complete` frame uses, so a cold launch rebuilds the reconnect
        # card from history instead of rendering the sentence and no affordance.
        if (fault := connection_fault(m.get("display_metadata"))) is not None:
            msg["connection"] = fault
        # Shared semantic truth for natively attributed rows.  Untyped legacy
        # rows keep their exact wire shape; their authorship is unresolved and
        # must not be guessed into an envelope during a read.
        if projection(m.get("display_metadata")) is not None:
            msg.update(owner_semantics(m))
        messages.append(msg)
    return messages
...
def _start_inflight_turn(session: dict, text: Any, *, display_kind: str | None = None) -> None:
    now = time.time()
    session["inflight_turn"] = {"assistant": "", "started_at": now, "streaming": True, "updated_at": now, "user": "" if display_kind == "hidden" else _inflight_text(text)}
...
def _fail_inflight_turn(
    session: dict, error: Any, error_surface: Optional[dict] = None, connection: Optional[dict] = None,
) -> None:
    ...
    ``connection`` rides along for the same reason ``error_surface`` does: a disconnected Connection
    outlives the socket that reported it, so a client that reconnects (or relaunches) must rebuild
    the reconnect card rather than fall back to rendering the raw ``error`` text."""
    ...
    if connection:
        turn["connection"] = dict(connection)
    else:
        turn.pop("connection", None)
