...
_METHODS = (
    ...
    "groups.rename", "groups.members", "groups.log", "groups.disband", "groups.replicate", "groups.replica_state",
    ...
    def dec(fn):
        def handler(rid, params: dict) -> dict:
            ...
            try:
                ...
                from tui_gateway.attachment_batches import RefusedBatch
                if isinstance(exc, RefusedBatch):
                    return _err(rid, code, str(exc), exc.data)
...
def _(rid, params: dict, _catalog=_local_catalog, _methods=_METHODS) -> dict:
    ...
    return _ok(rid, {
        ...
        "features": [
            ...
            "log_replication", "authority_takeover", "mutable_membership"],
...
def _(rid, params: dict, service) -> dict:
    ...
    if "attachment_batch" in params:
        from tui_gateway.attachment_batches import identity
        client_event_id = identity(client_event_id)
    event = service.send(
        room_id=params.get("room_id"), event_id=user_event_id(client_event_id),
        payload=params.get("payload"),
        **({"attachment_batch": params["attachment_batch"]} if "attachment_batch" in params else {}))
    return _ok(rid, {
...
@_room_method("groups.members", code=5118, room_code=4118, with_reason=True,
              service_code=4123, service_message=_WORKER_UNAVAILABLE)
def _(rid, params: dict, service) -> dict:
    """Set the desired roster while retaining removed identities for replay."""
    room = service.set_members(room_id=params.get("room_id"), event_id=params.get("event_id"), members=params.get("members"))
    return _ok(rid, {"room": room})
