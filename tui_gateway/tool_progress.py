...
def _on_tool_start(sid: str, tool_call_id: str, name: str, args: dict):
    _emit_activity(sid, "tool.start", {"tool_id": tool_call_id, "name": name})
...
def _on_tool_complete(sid: str, tool_call_id: str, name: str, args: dict, result: str):
    _emit_activity(sid, "tool.complete", {"tool_id": tool_call_id})
    ...
    if session is not None:
        from tui_gateway.interactive_progress import computer_notice
        if notice := computer_notice(session, tool_call_id, name, payload["result"]):
            # Material operational failure is not optional tool-progress chrome.
            with contextlib.suppress(Exception):
                _emit("notification.show", sid, notice)
    ...
    with contextlib.suppress(Exception):
        ...
        rendered: list[str] = []
        if _session_inline_diffs(sid) and render_edit_diff_with_delta(name, result, function_args=args, snapshot=snapshot, print_fn=rendered.append):
            payload["inline_diff"] = "\n".join(rendered)
