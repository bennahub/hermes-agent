...
_LONG_HANDLERS = frozenset({
    ...
    "process.list", "profiles.configure", "profiles.create", "profiles.delete", "profiles.describe",
    "profiles.get_asset",
...
def _emit(event: str, sid: str, payload: dict | None = None):
    ...
    _consider_push(event, sid, payload)
    if event in {"message.start", "message.delta", "thinking.delta", "reasoning.delta", "message.complete", "error"}:
        _emit_activity(sid, event, payload)
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    ...
    surfaces = {"project", "desktop_ui"} if platform == "desktop" else {"project"}
    from agent.skill_utils import parse_config_string_list
    disabled = ((_load_cfg().get("agent") or {}).get("disabled_toolsets") or [])
    if "connections" not in {name.strip() for name in parse_config_string_list(disabled)}:
        surfaces.add("connections")
    return surfaces
...
def _tool_lifecycle_required_for_ui(name: str) -> bool:
    ...
    return name in ("clarify", "setup_mcp", "request_connection")
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "show_reasoning": _load_show_reasoning(), "inline_diffs": _load_inline_diffs(), "source": _resolve_session_source(source),
...
def _deferred_session_record(
    ...
    return {
        ...
        "running": False, "session_key": session_key, "show_reasoning": _load_show_reasoning(), "inline_diffs": _load_inline_diffs(),
...
def _session_live_item(sid: str, session: dict, current_sid: str = "", *, activity_work_cache: dict | None = None) -> dict:
    ...
    activity = None
    try:
        from tui_gateway.activity import snapshot as activity_snapshot, work_state
        activity = activity_snapshot(session, status)
        home = session.get("profile_home")
        if activity_work_cache is None:
            activity["work_state"] = work_state(home)
        else:
            if home not in activity_work_cache:
                activity_work_cache[home] = work_state(home)
            activity["work_state"] = activity_work_cache[home]
    except Exception:
        # An optional projection must not break existing presence or expose
        # exception text (which may originate in a transport/provider).
        activity = None
...
def _respond(rid, params, key, *, allow_expired=False):
    ...
    with _prompt_lock:
        ...
        if batch is not None and not question_id and params.get(key, "") and len(batch["qids"]) == 1:
            # Older single-question renderers omit question_id even when the tool used the batch wire shape.
            # Preserve a non-empty answer for that one question; empty remains explicit cancel-all.
            qid = batch["qids"][0]
            batch["answers"][qid] = params[key]
            ev.set()
...
from . import (  # noqa: E402
    ...
    methods_projects as _methods_projects, owner_session_extensions as _owner_session_extensions,
...
for _m in (
    ...
    _owner_session_extensions, _methods_agent_computer):
