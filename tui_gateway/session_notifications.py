...
def _notification_home_scope(session: dict):
    """Keep native notification claims and acknowledgements in their session store."""
    ...
    expected = get_profile_dir(profile.name)
    root = get_default_hermes_root().resolve()
    if (not profile.is_absolute() or profile != expected or not profile.is_dir()
            or expected.parent.resolve() != root / "profiles"
            or profile.resolve() != root / "profiles" / profile.name):
        raise ValueError("notification profile home is not a canonical profile")
...
def _notif_submit(rid: str, sid: str, session: dict, text: str, what: str, **kwargs) -> bool:
    """message.start + _run_prompt_submit for a claimed turn; acknowledge actual admission."""
    previous_worker = session.get("_run_thread")
    try:
        ...
        if worker is not previous_worker and getattr(worker, "ident", None) is not None:
            # Thread.start can raise after launching. That worker owns the turn;
            # do not release its durable event for a duplicate dispatch.
            return True
...
def _notif_dispatch_event(sid: str, session: dict, evt: dict, text: str) -> None:
    """Bind profile storage across both native delivery paths, including failure release."""
    entered = False
    try:
        with _notification_home_scope(session):
            entered = True
            _notif_dispatch_event_in_home(sid, session, evt, text)
    except Exception:
        # Inner dispatch owns pre-admission release. Once a worker was admitted,
        # a failed durable ACK must not expose that live turn to another prompt.
        if not entered:
            _notif_release_turn(session)
        raise


def _notif_dispatch_event_in_home(sid: str, session: dict, evt: dict, text: str) -> None:
    ...
    try:
        ...
        raise
    if claim is None:
        # The caller claimed the idle turn before checking durable delivery.
        # A duplicate/competing delivery must not leave that session busy.
        _notif_release_turn(session)
        return
    from agent.message_projection import native_metadata, process_collaboration_identity
    metadata = _async_delegation_display_metadata(evt) if evt.get("type") == "async_delegation" else {}
    collaboration = {}
    if evt.get("session_id"):
        with contextlib.suppress(Exception):
            with _session_db(session) as db:
                collaboration = process_collaboration_identity(db, session.get("session_key"), evt["session_id"])
    kwargs = {"display_kind": "hidden", "display_metadata": native_metadata(
        "runtime", "internal", "control", metadata=metadata,
        source_session_id=evt.get("session_key") or session.get("session_key"),
        process_id=evt.get("session_id"), delegation_id=evt.get("delegation_id"), **collaboration)}
    try:
        admitted = _notif_submit(f"__notif__{int(time.time() * 1000)}", sid, session, text, "notification poller dispatch failed", **kwargs)
...
def _native_owned_completion_event(evt, registry) -> bool:
    """Drop a queued legacy completion when the process carries stable native ownership.

    The started-at match prevents a recycled process id or unrelated queued event
    from being hidden. Ownership fields are stamped at native bind time and remain
    valid even after the work row advances or is replaced.
    """
    ...
    try:
        ...
        return bool(
            process
            and getattr(process, "owner_continuity_work_id", "")
            and getattr(process, "owner_continuity_home", "")
            and evt.get("started_at") == getattr(process, "started_at", None)
        )
    except Exception:
        return False
...
def _notif_handle_event(sid, session, evt, emitted, registry, fmt, deferred) -> bool:
    ...
    if _notification_event_requires_owner(evt) and not _session_owns_notification_event(sid, session, evt):
        ...
        return True
    if evt_type == "completion" and _native_owned_completion_event(evt, registry):
        return True
...
def _notification_poller_loop(stop_event: threading.Event, sid: str, session: dict) -> None:
    # Raw worker threads do not inherit ContextVars from session initialization.
    with _notification_home_scope(session):
        _notification_poller_loop_in_home(stop_event, sid, session)


def _notification_poller_loop_in_home(stop_event: threading.Event, sid: str, session: dict) -> None:
