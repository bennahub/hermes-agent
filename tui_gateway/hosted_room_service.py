...
class HostedRoomService:
    ...
    def local_profiles(self) -> tuple[str, ...]:
        ...
        if profiles_dir.is_dir():
            profiles.update(
                ...
                if path.is_dir() and _PROFILE_ID_RE.fullmatch(path.name)
                and not named_profile_is_deleted(path))
    ...
    def _member_record(member: Any) -> dict[str, Any]:
        """The stored shape of one roster entry."""
        record: dict[str, Any] = {
        ...
        # Written only when it is False, so an unchanged roster serialises
        # byte-identically to one written before removal existed — which is
        # what keeps create_room's idempotency comparison working.
    ...
    def set_members(
        ...
        """Add or remove members without disturbing the room or its history.

        Removal marks the member inactive rather than dropping the entry. The
        log is full of events that name a ``member_id``, and replay resolves
        every one of them against the roster; deleting the entry would
        invalidate that member's history and take the conversation with it.
        An inactive member takes no further turns, which is the part the owner
        actually asked for.

        Re-adding someone reactivates the entry they already had, so a member
        who leaves and returns keeps one identity and one history rather than
        acquiring a second.
        """
        ...
        records = [self._member_record(member) for member in normalized]
        # Anyone the caller left out is carried forward inactive by
        # `set_room_members`, which is the only layer that sees both rosters.
        room = hosted_rooms.set_room_members(
            self.db_path,
            room_id=room_id,
            event_id=event_id,
            members=records,
        )
        # A removal can leave a queued turn belonging to someone who is no
        # longer participating; the next planning pass settles the room.
        self.runtime.wakeup()
        return room

    def send(self, *, room_id: str, event_id: str, payload: Any, attachment_batch=None) -> dict[str, Any]:
        if attachment_batch is None:
            return self._send(room_id=room_id, event_id=event_id, payload=payload)
        from tui_gateway import attachment_batches as batches
        import sqlite3
        if not isinstance(attachment_batch, dict):
            raise batches.BatchError("attachment_batch must be an object")
        mid = batches.identity(attachment_batch.get("batch_id"))
        if event_id != hosted_rooms.user_event_id(mid):
            raise batches.BatchError("Attachment event_id must equal batch_id")
        # Resolve native authority before reserving anything. Old releases could
        # append an event while leaving the upload row staged: that event wins.
        self._owned_authority(room_id)
        with hosted_rooms._transaction(self.db_path) as conn:
            existing = hosted_rooms._load_event(conn, room_id, event_id)
        if existing is not None:
            try:
                return self._send(room_id=room_id, event_id=event_id, payload=payload,
                                  attachment_batch=attachment_batch)
            except batches.RefusedBatch:
                # Canonical event presence always defeats a not-admitted proof,
                # including externally created or legacy conflicting events.
                raise batches.BatchError("This room event already exists; its attachment outcome must be checked") from None
        home, session = self.server._hermes_home, f"room:{room_id}"
        try:
            normalized = discussion.validate_user_payload(payload, has_uploads=True)
            if normalized.get("attachments"):
                raise batches.BatchError("Supply one attachment batch, not separate artifact references")
            room = discussion.validate_room(self._room(room_id), local_profiles=self.local_profiles())
            if any((m.target or {}).get("kind") == "peer" for m in room.active_members):
                raise batches.BatchError("Attachments require local room members; peer transport is text-only")
            batches.stage(home, "default", session, mid, mid, attachment_batch.get("items"))
            def dispatch(records, paths):
                referenced = {**normalized, "attachments": [
                    {key: record[key] for key in discussion._ATTACHMENT_FIELDS} for record in records]}
                event = self._send(room_id=room_id, event_id=event_id, payload=referenced)
                return {"result": {"event": event}}
            result = batches.submit(home, session, mid, mid, normalized, dispatch)
            if not isinstance(result.get("event"), dict):
                raise batches.BatchError("Attachment outcome is unknown; retry the same message identity")
            return {**result["event"], **({"idempotent": True} if result.get("replayed") else {})}
        except (batches.BatchError, discussion.DiscussionValidationError) as exc:
            # New callers claim in the same native index before append. Once a
            # competing caller crosses that boundary, seal_refusal cannot win.
            # Post-append/prepare failures stay uncertain; never rotate UUIDs.
            proof = exc if isinstance(exc, batches.RefusedBatch) else None
            if proof is None:
                try:
                    # The seal carries the reason, so a room upload refused for
                    # one named file says which file, exactly as a chat upload
                    # does. Without it the room path keeps the generic sentence
                    # and seals a tombstone with no reason at all.
                    proof = batches.seal_refusal(home, session, mid, mid,
                                                 batches.refusal_of(exc))
                except (batches.BatchError, OSError, sqlite3.Error):
                    pass
            if proof:
                raise proof from None
            raise

    def _send(self, *, room_id: str, event_id: str, payload: Any, attachment_batch=None) -> dict[str, Any]:
        normalized = discussion.validate_user_payload(payload, has_uploads=attachment_batch is not None)
        gateway_id, epoch = self._owned_authority(room_id)
        reply_to = normalized.get("reply_to")
        if reply_to is not None:
            delta = hosted_rooms.read_events(self.db_path, room_id=room_id, since_seq=reply_to - 1, limit=1)
            events = delta.get("events") or []
            if not events or int(events[0].get("seq", 0)) != int(reply_to):
                raise discussion.DiscussionValidationError("reply_to does not name an event in this room")
        if attachment_batch is not None or normalized.get("attachments"):
            from tui_gateway import attachment_batches
            room = discussion.validate_room(self._room(room_id), local_profiles=self.local_profiles())
            if any((m.target or {}).get("kind") == "peer" for m in room.active_members):
                raise discussion.DiscussionValidationError("Attachments require local room members; peer transport is text-only")
            home = self.server._hermes_home
            if attachment_batch is not None:
                if not isinstance(attachment_batch, dict) or normalized.get("attachments"):
                    raise discussion.DiscussionValidationError("Supply one attachment batch, not separate artifact references")
                mid = attachment_batches.identity(attachment_batch.get("batch_id"))
                if event_id != hosted_rooms.user_event_id(mid):
                    raise discussion.DiscussionValidationError("Attachment event_id must equal batch_id")
                receipt = attachment_batches.stage(home, "default", f"room:{room_id}", mid, mid,
                                                    attachment_batch.get("items"))
                normalized["attachments"] = [{k: r[k] for k in discussion._ATTACHMENT_FIELDS}
                                              for r in receipt["attachments"]]
            attachment_batches.room_records(home, room_id, normalized["attachments"])
